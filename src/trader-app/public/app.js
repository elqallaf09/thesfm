...
(() => {
  ...
  const { drawerTabLoading, drawerLoadStatus,
    ...
    marketUniverseRows, matchRec, lookupWatchlist: key => state.route.id === "watchlist" ? watchlistView.lookup(key) : null
  });
  ...
  function drawerTabContent(tab, context) {
    ...
    if (tab === "ai") return smartAnalysisTerminal(mergeRecLists(context.rec ? [context.rec] : [], [context.asset])[0], "drawer-analysis-terminal-title");
