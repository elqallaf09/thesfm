...
(function (global) {
  ...
  const observed = item => {
    const stamp = item?.lastUpdated || item?.dataTimestamp || item?.providerStatus?.lastUpdated;
  ...
  function render(item, options) {
    ...
    const rationale = a.explanation && typeof a.explanation === 'object' ? (a.explanation[options.lang] || a.explanation.en)
      : options.lang === 'ar' ? a.explanationAr : a.explanationEn;
    ...
    return `<section class="panel sa-research" aria-labelledby="${h(options.titleId)}">
      ...
      ${a.symbol ? `<button class="action-btn" type="button" data-symbol-details="${h(a.symbol)}">${h(t('فتح التحليل التفصيلي والأخبار', 'Open detailed analysis and news', 'Ouvrir l’analyse détaillée et les actualités'))}</button>` : ''}
    </section>`;
  }
