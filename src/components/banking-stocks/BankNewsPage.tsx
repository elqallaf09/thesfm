...
const CATEGORY_OPTIONS: Array<{ id: BankingCategory; ar: string; en: string; fr: string; sectors?: string[]; keywords?: string[] }> = [
  { id: 'all', ar: 'ÙƒÙ„ Ø§Ù„Ø£Ø®Ø¨Ø§Ø±', en: 'All news', fr: 'Toutes les actualitÃ©s' },
  { id: 'large_banks', ar: 'Ø§Ù„Ø¨Ù†ÙˆÙƒ Ø§Ù„ÙƒØ¨Ø±Ù‰', en: 'Large banks', fr: 'Grandes banques', sectors: ['large_banks'], keywords: ['bank', 'deposit', 'loan', 'lender'] },
  { id: 'regional_banks', ar: 'Ø§Ù„Ø¨Ù†ÙˆÙƒ Ø§Ù„Ø¥Ù‚Ù„ÙŠÙ…ÙŠØ©', en: 'Regional banks', fr: 'Banques rÃ©gionales', sectors: ['regional_banks'], keywords: ['regional bank', 'community bank'] },
  { id: 'investment_banks', ar: 'Ø¨Ù†ÙˆÙƒ Ø§Ù„Ø§Ø³ØªØ«Ù…Ø§Ø±', en: 'Investment banks', fr: 'Banques dâ€™investissement', sectors: ['investment_banks'], keywords: ['capital markets', 'ipo', 'deal', 'trading revenue'] },
  { id: 'payments', ar: 'Ø§Ù„Ù…Ø¯ÙÙˆØ¹Ø§Øª', en: 'Payments', fr: 'Paiements', sectors: ['payments'], keywords: ['payment', 'card', 'transaction', 'visa', 'mastercard'] },
  { id: 'asset_management', ar: 'Ø¥Ø¯Ø§Ø±Ø© Ø§Ù„Ø£ØµÙˆÙ„', en: 'Asset management', fr: 'Gestion dâ€™actifs', sectors: ['asset_management', 'exchanges_services'], keywords: ['asset management', 'wealth management', 'aum', 'exchange', 'ratings'] },
  { id: 'rates', ar: 'Ø§Ù„ÙØ§Ø¦Ø¯Ø© ÙˆØ§Ù„Ø³ÙŠÙˆÙ„Ø©', en: 'Rates and liquidity', fr: 'Taux et liquiditÃ©', keywords: ['interest rate', 'rates', 'fed', 'fomc', 'treasury', 'central bank', 'yield'] },
  { id: 'earnings', ar: 'Ø§Ù„Ù†ØªØ§Ø¦Ø¬ ÙˆØ§Ù„Ø£Ø±Ø¨Ø§Ø­', en: 'Earnings', fr: 'RÃ©sultats', keywords: ['earnings', 'revenue', 'profit', 'quarter', 'guidance', 'net income'] },
  { id: 'dividends', ar: 'Ø§Ù„ØªÙˆØ²ÙŠØ¹Ø§Øª ÙˆØ¥Ø¹Ø§Ø¯Ø© Ø§Ù„Ø´Ø±Ø§Ø¡', en: 'Dividends and buybacks', fr: 'Dividendes et rachats', keywords: ['dividend', 'buyback', 'capital return', 'repurchase'] },
  { id: 'regulation', ar: 'Ø§Ù„ØªÙ†Ø¸ÙŠÙ… ÙˆØ§Ù„Ù…Ø®Ø§Ø·Ø±', en: 'Regulation and risk', fr: 'RÃ©glementation et risque', keywords: ['regulation', 'regulator', 'basel', 'capital requirement', 'lawsuit', 'compliance', 'fine'] },
];

const COPY = {
  ar: {
    badge: 'Ø£Ø¨Ø­Ø§Ø« Ø§Ù„Ù‚Ø·Ø§Ø¹ Ø§Ù„Ø¨Ù†ÙƒÙŠ',
    title: 'Ø£Ø®Ø¨Ø§Ø± Ø§Ù„Ø¨Ù†ÙˆÙƒ',
    description: 'Ù…ØªØ§Ø¨Ø¹Ø© Ù…Ø±ÙƒØ²Ø© Ù„Ø£Ø³Ù‡Ù… Ø§Ù„Ø¨Ù†ÙˆÙƒ Ø§Ù„Ø¹Ø§Ù„Ù…ÙŠØ© ÙˆØ£Ø®Ø¨Ø§Ø± Ø§Ù„Ù‚Ø·Ø§Ø¹ ÙˆØ§Ù„Ù†ØªØ§Ø¦Ø¬ ÙˆØ§Ù„ØªÙˆØ²ÙŠØ¹Ø§Øª ÙˆØ§Ù„ØªÙ†Ø¸ÙŠÙ…Ø§Øª Ø§Ù„Ù…Ø¤Ø«Ø±Ø©.',
    stockUpdate: 'Ø¢Ø®Ø± ØªØ­Ø¯ÙŠØ« Ù„Ù„Ø£Ø³Ø¹Ø§Ø±',
    newsUpdate: 'Ø¢Ø®Ø± ØªØ­Ø¯ÙŠØ« Ù„Ù„Ø£Ø®Ø¨Ø§Ø±',
    providerConnected: 'Ø§Ù„Ø®Ø¯Ù…Ø© Ù…ØªØµÙ„Ø©',
    providerDelayed: 'Ø¨ÙŠØ§Ù†Ø§Øª Ù…ØªØ£Ø®Ø±Ø© Ø­Ø³Ø¨ Ù…Ø²ÙˆØ¯ Ø§Ù„Ø³ÙˆÙ‚',
    providerUnavailable: 'Ø§Ù„Ø®Ø¯Ù…Ø© ØºÙŠØ± Ù…ØªØ§Ø­Ø©',
    refresh: 'ØªØ­Ø¯ÙŠØ«',
    refreshing: 'Ø¬Ø§Ø±Ù Ø§Ù„ØªØ­Ø¯ÙŠØ«',
    marketStatus: 'Ø­Ø§Ù„Ø© Ø§Ù„Ø³ÙˆÙ‚',
    delayedQuote: 'Ù…ØªØ£Ø®Ø±Ø©',
    dataSource: 'Ù…Ø²ÙˆØ¯ Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª',
    newsSource: 'Ù…ØµØ§Ø¯Ø± Ø§Ù„Ø£Ø®Ø¨Ø§Ø±',
    bankingTicker: 'Ø´Ø±ÙŠØ· Ø£Ø³Ù‡Ù… Ø§Ù„Ø¨Ù†ÙˆÙƒ',
    marketSummary: 'Ù…Ù„Ø®Øµ Ø§Ù„Ù‚Ø·Ø§Ø¹ Ø§Ù„Ø¨Ù†ÙƒÙŠ',
    bestBank: 'Ø£ÙØ¶Ù„ Ø³Ù‡Ù…',
    weakestBank: 'Ø£Ø¶Ø¹Ù Ø³Ù‡Ù…',
    sectorAverage: 'Ù…ØªÙˆØ³Ø· Ø£Ø¯Ø§Ø¡ Ø§Ù„Ù‚Ø·Ø§Ø¹',
    risingStocks: 'Ø¹Ø¯Ø¯ Ø§Ù„Ø£Ø³Ù‡Ù… Ø§Ù„ØµØ§Ø¹Ø¯Ø©',
    fallingStocks: 'Ø¹Ø¯Ø¯ Ø§Ù„Ø£Ø³Ù‡Ù… Ø§Ù„Ù‡Ø§Ø¨Ø·Ø©',
    valuationRisk: 'Ù…ØªÙˆØ³Ø· Ø§Ù„ØªÙ‚ÙŠÙŠÙ… / Ø§Ù„Ù…Ø®Ø§Ø·Ø±Ø©',
    bankStocks: 'Ø£Ø³Ù‡Ù… Ø¨Ù†ÙƒÙŠØ© Ù„Ù„Ù…ØªØ§Ø¨Ø¹Ø©',
    bankStocksDescription: 'Ø¨Ø·Ø§Ù‚Ø§Øª Ù…Ø®ØªØµØ±Ø© ØªØ±Ø¨Ø· Ø§Ù„Ø³Ø¹Ø± Ø§Ù„Ø­Ø§Ù„ÙŠ Ø¨Ø§Ù„ØªØ­Ù„ÙŠÙ„ Ø§Ù„ÙÙ†ÙŠ Ø§Ù„Ø³Ø±ÙŠØ¹ ÙˆØ§Ù„ØªØ­Ù„ÙŠÙ„ Ø§Ù„ÙƒØ§Ù…Ù„.',
    showAllStocks: 'Ø¹Ø±Ø¶ ÙƒÙ„ Ø§Ù„Ø£Ø³Ù‡Ù…',
    showLessStocks: 'Ø¹Ø±Ø¶ Ø£Ù‚Ù„',
    quickAnalysis: 'ØªØ­Ù„ÙŠÙ„ Ø³Ø±ÙŠØ¹',
    fullAnalysis: 'Ø§Ù„ØªØ­Ù„ÙŠÙ„ Ø§Ù„ÙƒØ§Ù…Ù„',
    addWatchlist: 'Ø¥Ø¶Ø§ÙØ© Ù„Ù„Ù…Ø±Ø§Ù‚Ø¨Ø©',
    details: 'Ø§Ù„ØªÙØ§ØµÙŠÙ„',
    currentPrice: 'Ø§Ù„Ø³Ø¹Ø± Ø§Ù„Ø­Ø§Ù„ÙŠ',
    change: 'Ø§Ù„ØªØºÙŠØ±',
    market: 'Ø§Ù„Ø³ÙˆÙ‚',
    sector: 'Ø§Ù„ÙØ¦Ø©',
    dividendYield: 'Ø¹Ø§Ø¦Ø¯ Ø§Ù„ØªÙˆØ²ÙŠØ¹',
    marketCap: 'Ø§Ù„Ù‚ÙŠÙ…Ø© Ø§Ù„Ø³ÙˆÙ‚ÙŠØ©',
    unavailable: 'ØºÙŠØ± Ù…ØªØ§Ø­',
    featuredNews: 'Ø£Ù‡Ù… Ø£Ø®Ø¨Ø§Ø± Ø§Ù„Ø¨Ù†ÙˆÙƒ',
    readNews: 'Ù‚Ø±Ø§Ø¡Ø© Ø§Ù„Ø®Ø¨Ø±',
    translated: 'ØªØ±Ø¬Ù…Ø© Ø¢Ù„ÙŠØ©',
    originalText: 'Ø¹Ø±Ø¶ Ø§Ù„Ù†Øµ Ø§Ù„Ø£ØµÙ„ÙŠ',
    translatedText: 'Ø¹Ø±Ø¶ Ø§Ù„ØªØ±Ø¬Ù…Ø©',
    filters: 'Ø§Ù„Ø¨Ø­Ø« ÙˆØ§Ù„ØªØµÙÙŠØ©',
    searchPlaceholder: 'Ø§Ø¨Ø­Ø« ÙÙŠ Ø£Ø®Ø¨Ø§Ø± Ø§Ù„Ø¨Ù†ÙˆÙƒ Ø£Ùˆ Ø§Ù„Ø±Ù…Ø² Ø£Ùˆ Ø§Ù„Ù…ØµØ¯Ø±...',
    bankFilter: 'Ø§Ù„Ø¨Ù†Ùƒ / Ø§Ù„Ø±Ù…Ø²',
    allBanks: 'ÙƒÙ„ Ø§Ù„Ø¨Ù†ÙˆÙƒ',
    marketFilter: 'Ø§Ù„ÙØ¦Ø©',
    sourceFilter: 'Ø§Ù„Ù…ØµØ¯Ø±',
    allSources: 'ÙƒÙ„ Ø§Ù„Ù…ØµØ§Ø¯Ø±',
    periodFilter: 'Ø§Ù„ÙØªØ±Ø©',
    allPeriods: 'ÙƒÙ„ Ø§Ù„ÙØªØ±Ø§Øª',
    today: 'Ø§Ù„ÙŠÙˆÙ…',
    week: 'Ø¢Ø®Ø± 7 Ø£ÙŠØ§Ù…',
    month: 'Ø¢Ø®Ø± 30 ÙŠÙˆÙ…Ø§Ù‹',
    categoryFilter: 'Ù†ÙˆØ¹ Ø§Ù„Ø®Ø¨Ø±',
    sort: 'Ø§Ù„ØªØ±ØªÙŠØ¨',
    latest: 'Ø§Ù„Ø£Ø­Ø¯Ø«',
    oldest: 'Ø§Ù„Ø£Ù‚Ø¯Ù…',
    strongestMove: 'Ø£Ù‚ÙˆÙ‰ Ø­Ø±ÙƒØ© Ø³Ø¹Ø±ÙŠØ©',
    portfolioRelated: 'Ù…Ø±ØªØ¨Ø·Ø© Ø¨Ù…Ø­ÙØ¸ØªÙŠ',
    clearFilters: 'Ù…Ø³Ø­ Ø§Ù„ÙÙ„Ø§ØªØ±',
    resultCount: 'Ø¹Ø¯Ø¯ Ø§Ù„Ø£Ø®Ø¨Ø§Ø±',
    activeFilters: 'Ø§Ù„ÙÙ„Ø§ØªØ± Ø§Ù„Ù†Ø´Ø·Ø©',
    bankingNews: 'ØªØ¯ÙÙ‚ Ø§Ù„Ø£Ø®Ø¨Ø§Ø± Ø§Ù„Ø¨Ù†ÙƒÙŠØ©',
    bankingNewsDescription: 'Ø£Ø®Ø¨Ø§Ø± Ù…Ø®ØªØµØ±Ø© Ù…Ø¹ Ø§Ù„Ù…ØµØ¯Ø± ÙˆØ§Ù„ÙˆÙ‚Øª ÙˆØ§Ù„Ø±Ù…Ø² Ø§Ù„Ù…Ø±ØªØ¨Ø· ÙˆØªØ­Ù„ÙŠÙ„ Ø³Ø±ÙŠØ¹ Ø¹Ù†Ø¯ ØªÙˆÙØ±Ù‡.',
    analyzeRelatedStock: 'ØªØ­Ù„ÙŠÙ„ Ø§Ù„Ø³Ù‡Ù… Ø§Ù„Ù…Ø±ØªØ¨Ø·',
    save: 'Ø­ÙØ¸',
    share: 'Ù…Ø´Ø§Ø±ÙƒØ©',
    loadMore: 'Ø¹Ø±Ø¶ Ø§Ù„Ù…Ø²ÙŠØ¯',
    noResults: 'Ù„Ø§ ØªÙˆØ¬Ø¯ Ø£Ø®Ø¨Ø§Ø± Ù…Ø·Ø§Ø¨Ù‚Ø© Ù„Ù„ÙÙ„Ø§ØªØ± Ø§Ù„Ø­Ø§Ù„ÙŠØ©.',
    noResultsHint: 'Ø¬Ø±Ù‘Ø¨ ØªØ¹Ø¯ÙŠÙ„ ÙƒÙ„Ù…Ø§Øª Ø§Ù„Ø¨Ø­Ø« Ø£Ùˆ Ø¥Ø²Ø§Ù„Ø© Ø¨Ø¹Ø¶ Ø§Ù„ÙÙ„Ø§ØªØ±.',
    noNewsAvailable: 'Ù„Ø§ ØªÙˆØ¬Ø¯ Ø£Ø®Ø¨Ø§Ø± Ø¨Ù†ÙƒÙŠØ© Ù…ØªØ§Ø­Ø© Ø­Ø§Ù„ÙŠØ§Ù‹.',
    noNewsAvailableHint: 'Ù„Ù… ØªÙØ±Ø¬Ø¹ Ø§Ù„Ù…ØµØ§Ø¯Ø± Ø§Ù„Ø­Ø§Ù„ÙŠØ© Ø£Ø®Ø¨Ø§Ø±Ø§Ù‹ Ø¨Ù†ÙƒÙŠØ© Ù‚Ø§Ø¨Ù„Ø© Ù„Ù„Ø¹Ø±Ø¶. Ø­Ø§ÙˆÙ„ Ø§Ù„ØªØ­Ø¯ÙŠØ« Ù„Ø§Ø­Ù‚Ø§Ù‹.',
    tickerUnavailable: 'Ø¨ÙŠØ§Ù†Ø§Øª Ø´Ø±ÙŠØ· Ø§Ù„Ø£Ø³Ù‡Ù… ØºÙŠØ± Ù…ØªØ§Ø­Ø© Ø­Ø§Ù„ÙŠØ§Ù‹.',
    tickerUnavailableHint: 'Ø³ÙŠØ¸Ù‡Ø± Ø§Ù„Ø´Ø±ÙŠØ· Ø¹Ù†Ø¯ ØªÙˆÙØ± Ø£Ø³Ø¹Ø§Ø± Ø§Ù„Ø¨Ù†ÙˆÙƒ Ù…Ù† Ù…Ø²ÙˆØ¯ Ø§Ù„Ø³ÙˆÙ‚.',
    noSideData: 'Ù„Ø§ ØªÙˆØ¬Ø¯ Ø¨ÙŠØ§Ù†Ø§Øª ÙƒØ§ÙÙŠØ© Ø­Ø§Ù„ÙŠØ§Ù‹.',
    providerError: 'ØªØ¹Ø°Ø± ØªØ­Ø¯ÙŠØ« Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª Ø­Ø§Ù„ÙŠØ§Ù‹. ÙŠØªÙ… Ø¹Ø±Ø¶ Ø¢Ø®Ø± Ø¨ÙŠØ§Ù†Ø§Øª Ù…ØªØ§Ø­Ø© Ø¥Ù† ÙˆØ¬Ø¯Øª.',
    retry: 'Ø¥Ø¹Ø§Ø¯Ø© Ø§Ù„Ù…Ø­Ø§ÙˆÙ„Ø©',
    sideRailTitle: 'Ù„ÙˆØ­Ø© Ù…ØªØ§Ø¨Ø¹Ø© Ø§Ù„Ø¨Ù†ÙˆÙƒ',
    watchedStocks: 'Ø£ÙƒØ«Ø± Ø§Ù„Ø£Ø³Ù‡Ù… Ù…ØªØ§Ø¨Ø¹Ø©',
    topStories: 'Ø£Ù‡Ù… Ø§Ù„Ø£Ø®Ø¨Ø§Ø±',
    sources: 'Ù…ØµØ§Ø¯Ø± Ø§Ù„Ø£Ø®Ø¨Ø§Ø±',
    categories: 'ÙØ¦Ø§Øª Ø§Ù„Ø£Ø®Ø¨Ø§Ø±',
    sourceDisclaimer: 'Ø§Ù„Ø£Ø®Ø¨Ø§Ø± ÙˆØ§Ù„Ø£Ø³Ø¹Ø§Ø± Ù…Ø¬Ù…Ø¹Ø© Ù…Ù† Ù…Ø²ÙˆØ¯ÙŠÙ† Ø®Ø§Ø±Ø¬ÙŠÙŠÙ† ÙˆÙ‚Ø¯ ØªÙƒÙˆÙ† Ù…ØªØ£Ø®Ø±Ø©. Ù‡Ø°Ù‡ Ø§Ù„ØµÙØ­Ø© Ù„Ø£ØºØ±Ø§Ø¶ Ù…Ø¹Ù„ÙˆÙ…Ø§ØªÙŠØ© ÙˆØªØ¹Ù„ÙŠÙ…ÙŠØ© ÙÙ‚Ø· ÙˆÙ„Ø§ ØªØ¹Ø¯ ØªÙˆØµÙŠØ© Ø§Ø³ØªØ«Ù…Ø§Ø±ÙŠØ©.',
    quickTitle: 'Ù…Ø¹Ø§ÙŠÙ†Ø© Ø§Ù„ØªØ­Ù„ÙŠÙ„ Ø§Ù„Ø³Ø±ÙŠØ¹',
    quickLoading: 'Ø¬Ø§Ø±Ù ØªØ­Ù„ÙŠÙ„ Ø³Ù‡Ù…',
    quickError: 'ØªØ¹Ø°Ø± ØªØ­Ù„ÙŠÙ„ Ø§Ù„Ø³Ù‡Ù… Ø­Ø§Ù„ÙŠØ§Ù‹. Ø­Ø§ÙˆÙ„ Ù…Ø±Ø© Ø£Ø®Ø±Ù‰.',
    unsupportedSymbol: 'Ù‡Ø°Ø§ Ø§Ù„Ø±Ù…Ø² ØºÙŠØ± Ù…Ø¯Ø¹ÙˆÙ… Ù…Ù† Ù…Ø²ÙˆØ¯ Ø§Ù„ØªØ­Ù„ÙŠÙ„ Ø§Ù„Ø­Ø§Ù„ÙŠ.',
    close: 'Ø¥ØºÙ„Ø§Ù‚',
    updateAnalysis: 'ØªØ­Ø¯ÙŠØ« Ø§Ù„ØªØ­Ù„ÙŠÙ„',
    openFullAnalysis: 'ÙØªØ­ Ø§Ù„ØªØ­Ù„ÙŠÙ„ Ø§Ù„ÙƒØ§Ù…Ù„',
    reading: 'Ø§Ù„Ù‚Ø±Ø§Ø¡Ø© Ø§Ù„Ø­Ø§Ù„ÙŠØ©',
    confidence: 'Ø§Ù„Ø«Ù‚Ø©',
    risk: 'Ø§Ù„Ù…Ø®Ø§Ø·Ø±',
    shortTrend: 'Ø§Ù„Ø§ØªØ¬Ø§Ù‡ Ø§Ù„Ù‚ØµÙŠØ±',
    mediumTrend: 'Ø§Ù„Ø§ØªØ¬Ø§Ù‡ Ø§Ù„Ù…ØªÙˆØ³Ø·',
    rsi: 'RSI',
    macd: 'MACD',
    movingAverages: 'Ù…ØªÙˆØ³Ø· 20 / 50',
    support: 'Ø§Ù„Ø¯Ø¹Ù… Ø§Ù„Ø£Ù‚Ø±Ø¨',
    resistance: 'Ø§Ù„Ù…Ù‚Ø§ÙˆÙ…Ø© Ø§Ù„Ø£Ù‚Ø±Ø¨',
    explanation: 'ØªÙØ³ÙŠØ± Ù…Ø®ØªØµØ±',
    buy: 'Ø´Ø±Ø§Ø¡',
    sell: 'Ø¨ÙŠØ¹',
    neutral: 'Ù…Ø­Ø§ÙŠØ¯ / Ø§Ù†ØªØ¸Ø§Ø±',
    bullish: 'ØµØ§Ø¹Ø¯',
    bearish: 'Ù‡Ø§Ø¨Ø·',
    low: 'Ù…Ù†Ø®ÙØ¶Ø©',
    medium: 'Ù…ØªÙˆØ³Ø·Ø©',
    high: 'Ù…Ø±ØªÙØ¹Ø©',
    panelNote: 'Ø§Ù„ØªØ­Ù„ÙŠÙ„ Ø¢Ù„ÙŠ ÙˆØªØ¹Ù„ÙŠÙ…ÙŠ ÙˆÙ„Ø§ ÙŠÙ…Ø«Ù„ ØªÙˆØµÙŠØ© Ù…Ø§Ù„ÙŠØ© Ø£Ùˆ Ø¯Ø¹ÙˆØ© Ù„Ù„Ø´Ø±Ø§Ø¡ Ø£Ùˆ Ø§Ù„Ø¨ÙŠØ¹.',
    escHint: 'Ø§Ø¶ØºØ· Escape Ù„Ø¥ØºÙ„Ø§Ù‚ Ø§Ù„Ù…Ø¹Ø§ÙŠÙ†Ø©.',
    noFeatured: 'Ù„Ø§ ØªÙˆØ¬Ø¯ Ø£Ø®Ø¨Ø§Ø± Ø¨Ø§Ø±Ø²Ø© Ø­Ø§Ù„ÙŠØ§Ù‹.',
    latestQuote: 'Ø¢Ø®Ø± Ø³Ø¹Ø±',
  },
  en: {
  ...
  fr: {
    ...
    title: 'ActualitÃ©s bancaires',
    description: 'Couverture ciblÃ©e des actions bancaires, rÃ©sultats, dividendes et rÃ©glementation.',
    stockUpdate: 'Mise Ã  jour des cours',
    newsUpdate: 'Mise Ã  jour des actualitÃ©s',
    providerConnected: 'Service connectÃ©',
    providerDelayed: 'DonnÃ©es de marchÃ© diffÃ©rÃ©es',
    providerUnavailable: 'Service indisponible',
    refresh: 'Actualiser',
    refreshing: 'Actualisation',
    marketStatus: 'Ã‰tat du marchÃ©',
    delayedQuote: 'DiffÃ©rÃ©',
    dataSource: 'Fournisseur',
    newsSource: 'Sources',
    bankingTicker: 'Ticker bancaire',
    marketSummary: 'SynthÃ¨se bancaire',
    bestBank: 'Meilleure action',
    weakestBank: 'Action la plus faible',
    sectorAverage: 'Moyenne de performance du secteur',
    risingStocks: 'Actions haussiÃ¨res',
    fallingStocks: 'Actions baissiÃ¨res',
    valuationRisk: 'Ã‰valuation / risque',
    bankStocks: 'Actions bancaires Ã  suivre',
    bankStocksDescription: 'Cartes compactes reliant les cours Ã  lâ€™analyse rapide et complÃ¨te.',
    showAllStocks: 'Tout afficher',
    showLessStocks: 'RÃ©duire',
    quickAnalysis: 'Analyse rapide',
    fullAnalysis: 'Analyse complÃ¨te',
    addWatchlist: 'Ajouter Ã  la liste',
    details: 'DÃ©tails',
    ...
    featuredNews: 'Ã€ la une banques',
    readNews: 'Lire',
    translated: 'Traduction automatique',
    originalText: 'Voir lâ€™original',
    translatedText: 'Voir la traduction',
    filters: 'Recherche et filtres',
    searchPlaceholder: 'Rechercher actualitÃ©, symbole ou source...',
    bankFilter: 'Banque / symbole',
    allBanks: 'Toutes les banques',
    marketFilter: 'CatÃ©gorie',
    sourceFilter: 'Source',
    allSources: 'Toutes les sources',
    periodFilter: 'PÃ©riode',
    ...
    portfolioRelated: 'LiÃ© au portefeuille',
    clearFilters: 'Effacer',
    resultCount: 'Articles',
    activeFilters: 'Filtres actifs',
    bankingNews: 'Flux bancaire',
    bankingNewsDescription: 'Articles compacts avec source, heure, symbole liÃ© et analyse rapide.',
    analyzeRelatedStock: 'Analyser lâ€™action liÃ©e',
    save: 'Enregistrer',
    share: 'Partager',
    loadMore: 'Voir plus',
    noResults: 'Aucune actualitÃ© ne correspond aux filtres.',
    noResultsHint: 'Essayez de modifier la recherche ou les filtres.',
    noNewsAvailable: 'Aucune actualitÃ© bancaire nâ€™est disponible pour le moment.',
    noNewsAvailableHint: 'Les sources actuelles nâ€™ont renvoyÃ© aucun article bancaire exploitable. RÃ©essayez plus tard.',
    tickerUnavailable: 'Les donnÃ©es du ticker sont indisponibles pour le moment.',
    tickerUnavailableHint: 'Le ruban apparaÃ®tra lorsque les cours bancaires seront disponibles.',
    noSideData: 'DonnÃ©es encore insuffisantes.',
    providerError: 'Impossible de mettre Ã  jour les donnÃ©es. Les derniÃ¨res donnÃ©es disponibles restent affichÃ©es.',
    retry: 'RÃ©essayer',
    sideRailTitle: 'Suivi bancaire',
    watchedStocks: 'Actions suivies',
    topStories: 'Articles clÃ©s',
    sources: 'Sources',
    categories: 'CatÃ©gories',
    sourceDisclaimer: 'Les actualitÃ©s et cours proviennent de fournisseurs externes et peuvent Ãªtre diffÃ©rÃ©s. Informations Ã©ducatives uniquement.',
    quickTitle: 'AperÃ§u dâ€™analyse rapide',
    quickLoading: 'Analyse de',
    quickError: 'Impossible dâ€™analyser cette action. RÃ©essayez.',
    unsupportedSymbol: 'Ce symbole nâ€™est pas pris en charge par le fournisseur actuel.',
    close: 'Fermer',
    updateAnalysis: 'Actualiser lâ€™analyse',
    openFullAnalysis: 'Ouvrir lâ€™analyse complÃ¨te',
    ...
    medium: 'Moyen',
    high: 'Ã‰levÃ©',
    panelNote: 'Analyse automatisÃ©e Ã  but Ã©ducatif uniquement. Ce nâ€™est pas un conseil financier.',
    escHint: 'Appuyez sur Escape pour fermer.',
    noFeatured: 'Aucun article Ã  la une.',
  ...
  return (
    <article className="bankStockCard">
      <div className="bankStockHeader">
        ...
        <div>
          <h3>{item.name}</h3>
          <p><span dir="ltr">{item.symbol}</span> Â· {sectorLabel(item.sector, lang, text)}</p>
        </div>
      ...
      <div className={`filterGrid ${open ? 'open' : ''}`}>
        ...
        <label className="filterField">
          ...
          <select value={symbol} onChange={event => setSymbol(event.target.value)}>
            ...
            {stocks.map(item => (
              <option key={item.symbol} value={item.symbol}>{item.symbol} Â· {item.name}</option>
            ))}
          </select>
        </label>
        <label className="filterField">
          ...
          {stories.length > 0 ? (
            stories.slice(0, 5).map(item => (
              <a key={item.id} href={safeExternalUrl(item.url) || '#'} target={safeExternalUrl(item.url) ? '_blank' : undefined} rel="noopener noreferrer nofollow">
                ...
                <em>{item.source} Â· {relativeTime(item.publishedAt, locale)}</em>
              </a>
            ))
          ) : (
    ...
    <FocusTrapPanel onClose={onClose}>
      <div className="quickPanelHeader">
        <div>
          ...
          <p><span dir="ltr">{state.symbol}</span> Â· {state.exchange || result?.exchange || text.unavailable} Â· {text.panelNote}</p>
        </div>
    ...
    <NewsPageShell category="banking" className="bankingShell" dir={dir} wide>
      <main id="main-content" className="bankingWorkspace">
        ...
        <footer className="bankDisclaimer">
          ...
          <span> Â· {text.newsSource}: {sources.slice(0, 3).map(item => item.source).join('ØŒ ') || text.unavailable}</span>
        </footer>
      </main>
