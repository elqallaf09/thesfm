...
export function AuthForm() {
  ...
  const handleForgotPassword = async () => {
    if (resetStep === 'username') {
      ...
      if (!profileData || !profileData.email) {
        setError(t.noEmailFound + '. ' + t.contactSupport);
        setLoading(false);
        return;
      }

      if (!profileData.security_question) {
        setError(t.noSecurityQuestion + '. ' + t.contactSupport);
        setLoading(false);
        return;
      }
    ...
    if (resetStep === 'question') {
      ...
      if (!profileData || profileData.security_answer?.toLowerCase() !== securityAnswer.trim().toLowerCase()) {
        setError(t.wrongSecurityAnswer);
        setLoading(false);
        return;
      }
    ...
    if (resetStep === 'reset') {
      ...
      if (signInError) {
        setError(t.verificationError + ', ' + t.tryNow);
        setLoading(false);
        return;
      }
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isRegister) {
      ...
      if (!securityQuestion) {
        setError(t.securityQuestionRequired);
        return;
      }
      if (!securityAnswer.trim()) {
        setError(t.securityAnswerRequired);
        return;
      }
      if (password !== confirmPassword) {
        setError(t.passwordMismatch);
    ...
    if (!isRegister) {
      ...
      if (isEmail) {
        ...
        if (error) {
          setError(error.message || t.operationFailed);
          setLoading(false);
          return;
        }
      } else {
    ...
    <main dir={isArabic ? 'rtl' : 'ltr'} className="relative min-h-screen overflow-hidden bg-gradient-to-br from-blue-50 via-white to-blue-100 px-4 py-10 dark:from-blue-950 dark:via-slate-900 dark:to-blue-900">
      ...
      <div className="absolute top-4 left-4 z-10">
        <Select value={language} onValueChange={(v) => setLanguage(v as typeof language)}>
          <SelectTrigger className="w-[140px] bg-white/80 backdrop-blur border-blue-200">
        ...
        <div className="grid w-full items-center gap-12 md:grid-cols-[1.2fr_0.8fr]">
          <section className="space-y-8 text-center md:text-start">
            ...
            <div className="space-y-4">
              <h1 className="text-5xl font-bold tracking-tight text-blue-900 dark:text-blue-100 md:text-6xl">
                {t.title}
              </h1>
              <p className="max-w-xl text-xl leading-relaxed text-slate-700 dark:text-slate-300">
                {t.subtitle}
              </p>
            </div>
          </section>
            ...
            <CardContent>
                ...
                {!showForgotPassword ? (
                  <>
                    <div className="space-y-2">
                      <Label htmlFor="username">{isRegister ? t.username : t.usernameOrEmail}</Label>
                      <Input id="username" value={username} onChange={(event) => setUsername(event.target.value)} placeholder={isRegister ? t.enterUsername : t.enterUsernameOrEmail} dir="ltr" autoComplete="username" className="border-blue-200 focus:border-blue-500" />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="password">{t.password}</Label>
                      <Input id="password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} dir="ltr" autoComplete={isRegister ? 'new-password' : 'current-password'} className="border-blue-200 focus:border-blue-500" />
                    </div>
                    {!isRegister && (
                      <Button type="button" variant="link" className="p-0 h-auto text-blue-600 hover:text-blue-700 text-sm" onClick={() => { setShowForgotPassword(true); setError(''); setForgotPasswordSuccess(''); setResetStep('username'); }}>
                        <KeyRound className="h-4 w-4 ms-1" />
                        {isArabic ? 'نسيت كلمة المرور؟' : isFrench ? 'Mot de passe oublié?' : isChinese ? '忘记密码？' : 'Forgot password?'}
                      </Button>
                    )}
                  </>
                ) : (
                  <>
                    {resetStep === 'username' && (
                      <div className="space-y-2">
                        ...
                        <Input id="reset-username" value={resetUsername} onChange={(e) => setResetUsername(e.target.value)} placeholder={t.enterUsername} dir="ltr" autoComplete="username" className="border-blue-200 focus:border-blue-500" />
                      </div>
                    )}
                    {resetStep === 'question' && (
                      <div className="space-y-4">
                        <div className="p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg border border-blue-200 dark:border-blue-800">
                          <p className="text-sm font-medium text-blue-900 dark:text-blue-100">{getSecurityQuestionText(storedQuestion, isArabic)}</p>
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="security-answer">{t.securityAnswer}</Label>
                          <Input id="security-answer" value={securityAnswer} onChange={(e) => setSecurityAnswer(e.target.value)} dir={isArabic ? 'rtl' : 'ltr'} className="border-blue-200 focus:border-blue-500" />
                        </div>
                      </div>
                    )}
                    {resetStep === 'reset' && (
                      <div className="space-y-4">
                        <div className="space-y-2">
                          <Label htmlFor="new-password">{t.newPassword || 'كلمة المرور الجديدة'}</Label>
                          <Input id="new-password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} dir="ltr" autoComplete="new-password" className="border-blue-200 focus:border-blue-500" />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="confirm-password">{t.confirmPassword}</Label>
                          <Input id="confirm-password" type="password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} dir="ltr" autoComplete="new-password" className="border-blue-200 focus:border-blue-500" />
                        </div>
                      </div>
                    )}
                  </>
                ...
                {isRegister && !showForgotPassword && (
                  <>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input id="email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="example@domain.com" dir="ltr" autoComplete="email" className="border-blue-200 focus:border-blue-500" />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="age">{t.age}</Label>
                      <Input id="age" type="number" value={age} onChange={(event) => setAge(event.target.value)} placeholder="25" dir="ltr" min="10" max="120" className="border-blue-200 focus:border-blue-500" />
                    </div>
                    <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="security-answer-reg">{t.securityAnswer}</Label>
                      <Input id="security-answer-reg" value={securityAnswer} onChange={(e) => setSecurityAnswer(e.target.value)} placeholder={t.enterAnswer} className="border-blue-200 focus:border-blue-500" />
                    </div>
                  </>
                )}

                <Button type="submit" className="h-12 w-full bg-blue-600 text-base hover:bg-blue-700" disabled={loading}>
              ...
              {showForgotPassword && (
                <div className="mt-4 space-y-3">
                  <Button type="button" variant="ghost" className="w-full text-sm text-blue-600 hover:text-blue-700" onClick={() => { setShowForgotPassword(false); setForgotPasswordSuccess(''); setResetStep('username'); }}>
                    {t.back}
                  </Button>
                </div>
              )}
