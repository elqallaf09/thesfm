...
const TEXT = {
  ar: {
    ...
    whatChanged: 'شنو تغير؟', whyItMatters: 'ليش يهمك؟', whatToDo: 'شنو تسوي الآن؟',
  ...
  en: {
    ...
    whatChanged: 'What changed?', whyItMatters: 'Why it matters', whatToDo: 'What to do now',
  ...
  fr: {
    ...
    whatChanged: 'Qu’est-ce qui a changé ?', whyItMatters: 'Pourquoi c’est important', whatToDo: 'Que faire maintenant',
...
export function CrossWorkspaceEconomicBrief() {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/economic-intelligence/daily-brief?lang=${encodeURIComponent(locale)}`, { credentials: 'same-origin', headers: { Accept: 'application/json' } })
      ...
      .then(payload => { if (!cancelled) { setBrief(payload.brief ?? null); setActions(Array.isArray(payload.actions) ? payload.actions : []); setHighestPriority(payload.highestPriority ?? null); setNarrative(payload.narrative ?? null); setLoading(false); } })
