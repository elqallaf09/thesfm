...
const TEXT = {
  ar: {
    ...
    subtitle: 'توأمك المالي الحالي ومسارك المتوقع خلال 12 شهراً، مبني على بياناتك الفعلية.',
    ...
    incomplete: 'بعض مصادر البيانات غير متاحة، لذلك دقة التوقع أقل.',
  ...
  en: {
    ...
    subtitle: 'Your current financial twin and 12-month trajectory, built from your real financial data.',
    ...
    incomplete: 'Some data sources are unavailable, so forecast confidence is lower.',
  ...
  fr: {
    ...
    subtitle: 'Votre jumeau financier actuel et sa trajectoire sur 12 mois, à partir de vos données réelles.',
    ...
    months: 'mois',
    scenarios: 'Trajectoire sur 12 mois',
    stress: 'Stress',
    base: 'Base',
    ...
    incomplete: 'Certaines sources sont indisponibles, la confiance des prévisions est donc réduite.',
        ...
        <section>
          ...
          <div className="sfm-economic-scenarios">
            {(['stress', 'base', 'optimistic'] as const).map((id) => {
              const point = state.forecast!.scenarios[id].points.at(-1);
              return <article key={id}><strong>{text[id]}</strong><span>{text.endLiquidity}: {point ? formatCurrency(point.liquidBalance, snapshot.currency, locale) : '—'}</span><span>{text.endNetWorth}: {point ? formatCurrency(point.netWorth, snapshot.currency, locale) : '—'}</span></article>;
            })}
          </div>
        </section>
