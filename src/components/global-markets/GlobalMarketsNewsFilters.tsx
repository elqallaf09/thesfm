...
import { EMPTY_NEWS_FILTERS, type MarketNewsFilters } from '@/lib/market/globalMarketNewsRequest';

const COPY = {
  ar: { country: 'الدولة', exchange: 'البورصة', company: 'الشركة أو الرمز', region: 'منطقة الأخبار', language: 'لغة المصدر', source: 'المصدر', asset: 'نوع الأصل', from: 'من تاريخ', to: 'إلى تاريخ', sort: 'الترتيب', all: 'الكل', latest: 'الأحدث', relevance: 'الأكثر صلة', apply: 'تطبيق الفلاتر', reset: 'مسح الفلاتر', hint: 'اختر من القوائم ثم اضغط تطبيق الفلاتر. المصادر من الأخبار المتاحة، والمناطق تشمل الدول الموجودة في القوائم.', invalid: 'اختر فترة صحيحة لا تتجاوز 90 يوماً.', stock: 'الأسهم', forex: 'الفوركس', commodity: 'السلع والمعادن', crypto: 'العملات الرقمية', index: 'المؤشرات', GULF: 'الخليج', ARAB: 'العالم العربي', MIDDLE_EAST: 'الشرق الأوسط', CHINA_HONGKONG: 'الصين وهونغ كونغ', ASIA: 'آسيا', NORTH_AMERICA: 'أمريكا الشمالية', GLOBAL: 'العالم' },
  en: { country: 'Country', exchange: 'Exchange', company: 'Company or symbol', region: 'News region', language: 'Source language', source: 'Source', asset: 'Asset type', from: 'From', to: 'To', sort: 'Sort', all: 'All', latest: 'Latest', relevance: 'Most relevant', apply: 'Apply filters', reset: 'Clear filters', hint: 'Choose from the lists, then apply your filters. Sources come from available news; regions cover the listed countries.', invalid: 'Choose a valid date range of up to 90 days.', stock: 'Stocks', forex: 'Forex', commodity: 'Commodities', crypto: 'Crypto', index: 'Indices', GULF: 'Gulf', ARAB: 'Arab world', MIDDLE_EAST: 'Middle East', CHINA_HONGKONG: 'China & Hong Kong', ASIA: 'Asia', NORTH_AMERICA: 'North America', GLOBAL: 'Global' },
  fr: { country: 'Pays', exchange: 'Bourse', company: 'Société ou symbole', region: 'Région', language: 'Langue source', source: 'Source', asset: 'Type d’actif', from: 'Du', to: 'Au', sort: 'Tri', all: 'Tous', latest: 'Plus récentes', relevance: 'Plus pertinentes', apply: 'Appliquer les filtres', reset: 'Effacer les filtres', hint: 'Choisissez dans les listes, puis appliquez les filtres. Les sources proviennent des actualités disponibles ; les régions couvrent les pays proposés.', invalid: 'Choisissez une période valide de 90 jours maximum.', stock: 'Actions', forex: 'Forex', commodity: 'Matières premières', crypto: 'Crypto', index: 'Indices', GULF: 'Golfe', ARAB: 'Monde arabe', MIDDLE_EAST: 'Moyen-Orient', CHINA_HONGKONG: 'Chine et Hong Kong', ASIA: 'Asie', NORTH_AMERICA: 'Amérique du Nord', GLOBAL: 'Monde' },
} as const;
...
export function GlobalMarketsNewsFilters({ lang, value, sources, onApply }: {
  ...
  function update(key: keyof MarketNewsFilters, next: string) {
    setDraft(current => ({ ...current, [key]: next, ...(key === 'country' ? { exchange: '', symbol: '', region: '' } : key === 'exchange' ? { symbol: '', region: '' } : key === 'region' ? { country: '', exchange: '', symbol: '' } : {}) }));
    setInvalid(false);
  }
  return (
