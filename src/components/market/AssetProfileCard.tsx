...
function textDirection(value: string | null | undefined): 'ltr' | undefined {
...
function Metric({ label, value }: { label: string; value: string | null | undefined }) {
  ...
  return (
    <div className="asset-profile-metric">
      <span>{label}</span>
      <strong dir={textDirection(safeValue)}>{safeValue}</strong>
    </div>
  );
}
