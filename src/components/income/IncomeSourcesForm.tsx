    ...
    <main dir="rtl" className="relative min-h-screen overflow-hidden bg-[linear-gradient(135deg,_#f7faf7_0%,_#eef6ef_42%,_#dfeee7_100%)] px-4 py-8 dark:bg-[linear-gradient(135deg,_#07110d_0%,_#0d1d16_48%,_#111827_100%)]">
      ...
      <div className="relative mx-auto max-w-5xl space-y-6">
        <div className="space-y-2 text-center">
          <p className="text-sm font-bold text-blue-700 dark:text-blue-300">مرحباً {username || 'بك'}</p>
          <h1 className="text-3xl font-bold text-blue-950 dark:text-blue-100">أنواع الدخل الشهري</h1>
          <p className="text-slate-600 dark:text-slate-300">اختر مصادر دخلك وأدخل قيمة كل مصدر ليتم حساب دخلك الشهري بدقة.</p>
        </div>

        <Card className="border-blue-900/10 bg-white/85 shadow-[0_24px_80px_rgba(30,58,138,0.14)] backdrop-blur-2xl dark:border-white/10 dark:bg-slate-950/70">
          <CardHeader>
          ...
          <CardContent className="space-y-5">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {INCOME_CATEGORIES.map((category) => (
                <section key={category.id} className="rounded-2xl border border-blue-900/10 bg-white/80 p-4 dark:border-white/10 dark:bg-slate-900/70">
                  <div className="space-y-3">
                    <div>
                      <h2 className="font-bold text-blue-950 dark:text-blue-100">{category.nameAr}</h2>
                      <p className="text-xs text-slate-500 dark:text-slate-400">{category.nameEn}</p>
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                      {category.examples.map((example) => (
                        <span key={example} className="rounded-full bg-blue-50 px-2 py-1 text-xs text-blue-800 dark:bg-blue-950/50 dark:text-blue-200">{example}</span>
                      ))}
                    </div>
            ...
            <div className="rounded-2xl border border-blue-300 bg-blue-50 p-5 text-center dark:border-blue-800 dark:bg-blue-950/40">
              <span className="text-sm text-blue-700 dark:text-blue-300">إجمالي المدخول الشهري</span>
              <p className="text-4xl font-bold text-blue-900 dark:text-blue-100">{totalIncome.toLocaleString('ar-SA')} د.ك</p>
            </div>

            <Button onClick={saveSources} disabled={saving} className="h-12 w-full bg-blue-700 text-base hover:bg-blue-800">
