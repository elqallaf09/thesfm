...
function SkeletonList({ title }: { title: string }) {
  return (
...
function MoverRow({
  ...
  return (
    <article className="market-movers-row">
...
function MoverList({
  ...
  return (
    <section className={`market-movers-list tone-${config.tone}`}>
      <div className="market-movers-list-title">
        <span>
          <Icon size={17} />
        </span>
...
export function MarketMoversCard({ market, marketLabel, locale, labels }: MarketMoversCardProps) {
  ...
  const listConfigs = useMemo<MoverListConfig[]>(() => [
  ...
  return (
    <section className="market-movers-card" aria-label={labels.title}>
      <div className="market-movers-header">
        ...
        >
          <RefreshCcw size={16} className={refreshing ? 'spinning' : undefined} />
          {labels.refresh}
        </button>
      </div>
      ...
      {loading ? (
        <div className="market-movers-grid">
          {listConfigs.map(config => <SkeletonList key={config.key} title={config.label} />)}
        </div>
      ) : !hasData(data) ? (
        <div className="market-movers-unavailable">
      ...
      <style jsx global>{`
        ...
        .market-movers-header{display:flex;align-items:flex-start;justify-content:space-between;gap:14px;min-width:0}
        .market-movers-title{display:flex;align-items:center;gap:13px;min-width:0}
        .market-movers-icon{width:56px;height:56px;flex:0 0 auto;border-radius:18px;display:grid;place-items:center;background:#CCFBF1;border:1px solid rgba(15,118,110,.25);color:#0F766E}
        .dark .market-movers-icon{background:rgba(47,214,192,.12);border-color:rgba(47,214,192,.25);color:#2FD6C0}
        .market-movers-title p{margin:0;color:var(--movers-accent);font-size:12px;font-weight:950;line-height:1.4}
        .market-movers-title h2{margin:3px 0;color:var(--movers-text);font-size:24px;font-weight:950;line-height:1.2}
        .market-movers-title span{display:block;color:var(--movers-muted);font-size:13px;font-weight:850;line-height:1.65}
        .market-movers-refresh{flex:0 0 auto;min-height:42px;border:1px solid var(--movers-border-strong);border-radius:15px;background:rgba(47,214,192,.10);color:var(--movers-text);display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:0 13px;font:950 12px Tajawal,Arial,sans-serif;cursor:pointer;transition:transform .18s ease,border-color .18s ease,background .18s ease,color .18s ease}
        .market-movers-refresh:hover,.market-movers-refresh:focus-visible{outline:none;transform:translateY(-1px);border-color:rgba(47,214,192,.55);background:rgba(47,214,192,.16);color:var(--movers-accent)}
        .market-movers-refresh:disabled{opacity:.68;cursor:not-allowed;transform:none}
        .market-movers-meta{display:flex;align-items:center;gap:10px;flex-wrap:wrap;color:var(--movers-muted);font-size:12px;font-weight:900}
        .market-movers-meta span,.market-movers-meta em{display:inline-flex;align-items:center;gap:5px;border:1px solid var(--movers-border);border-radius:999px;background:rgba(142,166,195,.08);padding:7px 10px;font-style:normal;line-height:1.2}
        ...
        .market-movers-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;min-width:0}
        .market-movers-list{min-width:0;border:1px solid var(--movers-border);border-radius:20px;background:rgba(255,255,255,.48);padding:14px;display:grid;gap:12px;overflow:hidden}
        .dark .market-movers-list{background:rgba(15,29,49,.42)}
        .market-movers-list-title{display:flex;align-items:center;gap:9px;min-width:0}
        .market-movers-list-title span{width:34px;height:34px;border-radius:13px;display:grid;place-items:center;flex:0 0 auto;background:rgba(47,214,192,.12);color:var(--movers-accent)}
        .market-movers-list-title strong{min-width:0;color:var(--movers-text);font-size:14px;font-weight:950;line-height:1.4}
        .market-movers-list-body{display:grid;gap:10px;min-width:0}
        .market-movers-row{min-width:0;border:1px solid var(--movers-border);border-radius:16px;background:linear-gradient(180deg,rgba(255,255,255,.68),rgba(255,255,255,.38));padding:11px;display:grid;grid-template-columns:auto minmax(0,1fr);gap:10px;align-items:start}
        .dark .market-movers-row{background:linear-gradient(180deg,rgba(19,36,58,.76),rgba(15,29,49,.54))}
        .market-movers-rank{width:28px;height:28px;border-radius:999px;background:#E0F2FE;color:#075985;display:grid;place-items:center;font-size:12px;font-weight:950;line-height:1}
        .dark .market-movers-rank{background:rgba(47,214,192,.12);color:#2FD6C0}
        .market-movers-identity{min-width:0;display:grid;gap:3px}
        .market-movers-identity strong{color:var(--movers-text);font-size:14px;font-weight:950;letter-spacing:.02em;line-height:1.25}
        .market-movers-identity span{color:var(--movers-muted);font-size:11px;font-weight:850;line-height:1.45;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
        .market-movers-values{grid-column:1/-1;margin:0;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px}
        .market-movers-values div{min-width:0;border-radius:12px;background:rgba(142,166,195,.08);padding:7px}
        .market-movers-values dt{color:var(--movers-muted);font-size:10px;font-weight:900;line-height:1.4}
        .market-movers-values dd{margin:2px 0 0;color:var(--movers-text);font-size:11px;font-weight:950;line-height:1.3;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
        ...
        .market-movers-list-empty{margin:0;color:var(--movers-muted);font-size:12px;font-weight:850;line-height:1.7;padding:18px 6px;text-align:center}
        .market-movers-unavailable{display:grid;place-items:center;gap:10px;text-align:center;border:1px dashed var(--movers-border-strong);border-radius:20px;padding:34px 18px;color:var(--movers-muted);background:rgba(142,166,195,.07)}
        .market-movers-unavailable svg{color:#B45309}.dark .market-movers-unavailable svg{color:#F5B942}
        .market-movers-unavailable strong{color:var(--movers-text);font-size:17px;font-weight:950}
        .market-movers-unavailable p{margin:0;max-width:640px;color:var(--movers-muted);font-size:13px;font-weight:850;line-height:1.75}
        ...
        .market-movers-list.skeleton .market-movers-row i{width:28px;height:28px}
        .market-movers-list.skeleton .market-movers-row span{width:70%;height:14px}
        .market-movers-list.skeleton .market-movers-row b{width:54px;height:14px}
        @keyframes marketMoversShimmer{to{background-position:-220% 0}}
        @media(max-width:1180px){.market-movers-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
        @media(max-width:720px){.market-movers-card{border-radius:20px;padding:16px}.market-movers-header{display:grid}.market-movers-refresh{width:100%}.market-movers-grid{grid-template-columns:1fr}.market-movers-title h2{font-size:21px}.market-movers-values{grid-template-columns:1fr}.market-movers-identity span{white-space:normal}}
      `}</style>
    </section>
  );
}
