...
function marketSourceLabel(source?: string | null, fallback = 'Yahoo Finance') {
  ...
  if (!clean || isTechnicalProviderSource(clean)) return fallback;
  return clean;
}

function formatProviderStatusForDebug(source?: string | null, fallback = false) {
  const label = marketSourceLabel(source, fallback ? 'غير متاح' : 'غير متاح');
...
export default function LegacyMarketAnalysisWorkspace() {
  ...
  const { access: adminAccess } = useAdminAccess(activeTab === 'economicCalendar' ? user?.id : null);
  const canViewMarketDiagnostics = adminAccess.isAdmin
    && (adminAccess.isSuperAdmin || adminAccess.permissions.admin_dashboard === true);
  ...
  const selectedDataStatusLabel = selected
    ...
    : t('market_badge_live');
  const selectedProviderStatus = selected ? formatProviderStatusForDebug(selected.source ?? selected.provider, Boolean((selected as { fallback?: boolean }).fallback)) : null;
  const marketUnavailableBadge = lang === 'ar' ? marketUnavailableBadgeAr : t('market_unavailable');
  const marketNoDataLabel = lang === 'ar' ? marketNoDataLabelAr : t('market_unavailable');
  useEffect(() => {
    if (!isMarketAnalysisDev || !selected || !selectedProviderStatus) return;
    if (selectedProviderStatus === marketUnavailableBadge) return;
    console.debug('[market-analysis] provider status', {
  ...
  const assetSnapshot = selected ? {
    ...
    priceChangeLabel: marketUnavailableBadge,
    quoteTimestamp: null,
    quoteTimestampLabel: marketNoDataLabel,
    dataStatus: 'unavailable',
    dataStatusLabel: marketUnavailableBadge,
  ...
  return (
    <div className="market-shell" dir={dir}>
      <WorkspacePageContainer as="main" variant="full" className="market-main">
        ...
        {activeTab === 'economicCalendar' && (
          <EconomicCalendarPanel
            ...
            onRefresh={loadEconomicCalendar}
            canViewDiagnostics={canViewMarketDiagnostics}
          />
        )}
              ...
              <aside className="market-panel">
                ...
                {hasFundamentals ? (
                  <div className="indicator-list">
                    ...
                    <MarketMetric label={t('market_data_status')} value={selectedDataStatusLabel} />
                  </div>
                ) : (
            ...
            <section className="market-tools-grid">
              ...
              <article className="market-panel" id="market-alerts">
                ...
                <div className="alert-form">
                  <label htmlFor="market-alert-type-overview" className="sr-only">{t('market_alert_type_label')}</label>
                  <select id="market-alert-type-overview" value={alertType} onChange={event => setAlertType(normalizeAlertType(event.target.value))}>
                  ...
                  <label htmlFor="market-alert-value-overview" className="sr-only">{t('market_alert_value_label')}</label>
                  <input id="market-alert-value-overview" aria-label={t('market_alert_value_label')} value={alertThreshold} inputMode="decimal" onChange={event => setAlertThreshold(event.target.value)} disabled={alertType === 'rsi_above' || alertType === 'rsi_below'} />
              ...
              <article className="market-panel">
                ...
                <button type="button" className="report-button" aria-expanded={reportOpen} aria-controls="market-asset-report-overview" onClick={() => setReportOpen(open => !open)}>
                  <FileText size={16} />{t('market_generate_ai_report')}
                </button>
                {reportOpen && (
                  <div id="market-asset-report-overview" className="asset-report">
          ...
          <section className="market-panel market-focused-tab">
            {activeTab === 'alerts' && (
              <>
                ...
                <div className="alert-form">
                  <label htmlFor="market-alert-type-focused" className="sr-only">{t('market_alert_type_label')}</label>
                  <select id="market-alert-type-focused" value={alertType} onChange={event => setAlertType(normalizeAlertType(event.target.value))}>
                    <option value="above">{t('market_alert_above')}</option>
                    <option value="below">{t('market_alert_below')}</option>
                    <option value="change_exceeds">{t('market_alert_change')}</option>
                    <option value="rsi_above">{t('market_alert_rsi_above')}</option>
                  ...
                  <label htmlFor="market-alert-value-focused" className="sr-only">{t('market_alert_value_label')}</label>
                  <input id="market-alert-value-focused" aria-label={t('market_alert_value_label')} value={alertThreshold} inputMode="decimal" onChange={event => setAlertThreshold(event.target.value)} disabled={alertType === 'rsi_above' || alertType === 'rsi_below'} />
                  <button type="button" onClick={() => void saveAlert()}><Plus size={15} />{t('market_create_alert')}</button>
                </div>
                <div className="saved-alerts">
            ...
            {activeTab === 'assetReport' && (
              <>
                ...
                <button type="button" className="report-button" aria-expanded={reportOpen} aria-controls="market-asset-report-focused" onClick={() => setReportOpen(open => !open)}>
                  <FileText size={16} />{t('market_generate_ai_report')}
                </button>
                {reportOpen && (
                  <div id="market-asset-report-focused" className="asset-report">
                    {reportLines.map(line => <p key={line}>{line}</p>)}
                    <small>{t('market_disclaimer')}</small>
                  </div>
                )}
      ...
      <style jsx>{`
        .market-shell{min-height:100dvh;background:var(--surface);color:var(--foreground);overflow-x:hidden;font-family:var(--font-ui)}
        .market-main{width:100%;max-width:none;min-width:0;display:grid;gap:16px;overflow-x:hidden;box-sizing:border-box}.market-main>*{width:100%;max-width:none;min-width:0;box-sizing:border-box}
        .market-hero{position:relative;overflow:visible;border-radius:var(--radius-panel);background:var(--surface-muted);color:var(--surface);padding:28px;display:grid;grid-template-columns:minmax(0,1fr) 260px;gap:20px;align-items:end;box-shadow:var(--shadow-card);border:1px solid color-mix(in srgb,var(--hero-foreground) 28%,transparent)}
        .market-hero:before{content:"";position:absolute;inset-inline-end:-70px;top:-80px;width:230px;height:230px;border-radius:var(--radius-pill);background:var(--accent-soft);filter:blur(18px)}
        .market-hero-copy,.market-hero-card{position:relative;z-index:1}.market-eyebrow{display:inline-flex;align-items:center;gap:8px;border:1px solid color-mix(in srgb,var(--hero-foreground) 28%,transparent);background:color-mix(in srgb,var(--hero-foreground) 10%,transparent);color:var(--hero-foreground);border-radius:var(--radius-pill);padding:6px 12px;font-size:12px;font-weight:600;margin-bottom:14px}
        .market-hero h1{margin:0 0 10px;font-size:clamp(30px,5vw,52px);line-height:1.02;font-weight:600}.market-hero p{max-width:760px;margin:0;color:var(--hero-foreground-muted);line-height:1.8;font-size:14px}
        .market-search-panel{margin-top:22px;display:grid;grid-template-columns:minmax(0,1.6fr) minmax(160px,.7fr) minmax(150px,.65fr) max-content;gap:10px;align-items:end}.market-search-panel label{display:grid;gap:7px}.market-search-panel label>span{font-size:12px;font-weight:600;color:var(--hero-foreground)}.market-search-field{position:relative}.market-search-panel label>div{height:var(--control-h-lg);display:flex;align-items:center;gap:9px;border:1px solid var(--border);border-radius:var(--radius-card);background:var(--surface);padding:0 13px;color:var(--foreground);box-shadow:var(--shadow-card)}.market-search-panel label>div:focus-within{border-color:var(--accent);box-shadow:var(--focus-shadow)}.market-search-panel label>div svg{color:var(--primary);flex-shrink:0}.market-search-panel input,.market-search-panel select{width:100%;height:var(--control-h-lg);min-width:0;border:1px solid var(--border-strong);border-radius:var(--radius-card);background:var(--surface);color:var(--foreground);padding:0 13px;font:400 13px var(--font-ui);outline:0}.market-search-panel input{border:0;background:transparent;padding:0;color:var(--foreground)}.market-search-panel input::placeholder{color:var(--foreground-muted);opacity:1}.market-search-panel select{cursor:pointer;box-shadow:var(--shadow-card)}.market-search-panel select:focus{border-color:var(--accent);box-shadow:var(--shadow-card)}.market-search-panel select option{color:var(--foreground);background:var(--surface)}.market-search-panel button{height:var(--control-h-lg);border:0;border-radius:var(--radius-card);background:var(--surface);color:var(--foreground);padding:0 16px;display:inline-flex;align-items:center;justify-content:center;gap:8px;font:600 13px var(--font-ui);cursor:pointer;box-shadow:var(--shadow-card)}.market-search-panel button svg{color:var(--foreground)}.market-search-panel button:disabled{opacity:.9;cursor:wait;background:var(--surface);color:var(--foreground)}.market-search-results{position:absolute;z-index:90;inset-inline:0;top:calc(100% + 12px);max-height:min(320px,48dvh);overflow-y:auto;background:var(--surface);color:var(--foreground);border:1px solid var(--border);border-radius:var(--radius-control);box-shadow:var(--shadow-popover);padding:10px;display:grid;gap:8px;overscroll-behavior:contain}.market-search-results strong{color:var(--foreground);font-size:12px;font-weight:600;padding:5px 7px}.market-search-results p{margin:0;color:var(--foreground-muted);font-size:13px;font-weight:600;line-height:1.7;padding:12px}.market-search-results button{height:auto;min-height:58px;width:100%;border:1px solid var(--border-strong);background:var(--surface);color:var(--foreground);border-radius:var(--radius-control);padding:11px 12px;display:flex;align-items:center;justify-content:space-between;gap:12px;text-align:start;box-shadow:none}.market-search-results button:hover,.market-search-results button:focus-visible,.market-search-results button[aria-selected="true"]{background:var(--primary);border-color:var(--focus-ring);outline:none;box-shadow:var(--focus-shadow)}.market-search-results button span{display:grid;gap:4px;min-width:0}.market-search-results button b{font-size:15px;color:var(--foreground);letter-spacing:.02em;direction:ltr;unicode-bidi:isolate}.market-search-results button em{font-style:normal;color:var(--primary-hover);font-size:13px;line-height:1.45;white-space:normal;overflow-wrap:anywhere}.market-search-results button small{color:var(--primary-hover);background:var(--surface);border:1px solid var(--border-strong);border-radius:var(--radius-pill);padding:4px 8px;font-size:12px;font-weight:600;line-height:1.3;flex-shrink:0;white-space:nowrap}
        .market-search-field{z-index:70;min-width:0}.market-search-panel .market-search-input-shell{position:relative;z-index:2}.market-search-results{z-index:9999;top:calc(100% + 10px);max-height:min(360px,52dvh);overflow-y:auto;overflow-x:hidden;background:var(--surface);border-color:var(--border);border-radius:var(--radius-card);box-shadow:var(--shadow-popover);padding:8px;gap:6px}.market-search-results strong{padding:6px 8px;color:var(--foreground);line-height:1.4}.market-search-results p{border-radius:var(--radius-control);background:var(--surface);text-align:start}.market-search-results button{min-height:68px;display:grid;grid-template-columns:1fr;align-items:center;justify-items:stretch;gap:7px;padding:11px 12px;border-radius:var(--radius-control);background:var(--surface);border-color:var(--border-strong);text-align:start;color:var(--foreground);box-shadow:none}.market-search-results button:hover,.market-search-results button:focus-visible,.market-search-results button[aria-selected="true"]{background:var(--primary);border-color:var(--focus-ring);box-shadow:var(--focus-shadow);outline:none}.market-search-result-main{display:flex!important;align-items:center;justify-content:space-between;gap:12px;min-width:0}.market-search-results button b{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;direction:inherit;unicode-bidi:normal;letter-spacing:0;color:var(--foreground);font-size:14px}.market-search-results button em{flex-shrink:0;border-radius:var(--radius-pill);border:1px solid var(--border-strong);background:var(--surface);color:var(--primary-hover);padding:4px 8px;font-style:normal;font-size:12px;font-weight:700;line-height:1.2;direction:ltr;unicode-bidi:isolate}.market-search-results button small{display:block;width:100%;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding:0;border:0;background:transparent;color:var(--foreground-muted);font-size:12px;line-height:1.55;text-align:start}
        .market-search-field{position:relative;z-index:100;display:grid;gap:7px;min-width:0;overflow:visible}.market-search-field>label{font-size:12px;font-weight:600;color:var(--hero-foreground);line-height:1.4}.market-search-combobox{position:relative;width:100%;overflow:visible}.market-search-panel .market-search-input-shell{height:var(--control-h-lg);display:flex;align-items:center;gap:9px;border:1px solid var(--border-strong);border-radius:var(--radius-card);background:var(--surface);padding:0 13px;color:var(--foreground);box-shadow:var(--shadow-card)}.market-search-submit{height:var(--control-h-lg);border:0;border-radius:var(--radius-card);background:var(--primary);color:var(--foreground);padding:0 16px;display:inline-flex;align-items:center;justify-content:center;gap:8px;font:600 13px var(--font-ui);cursor:pointer;box-shadow:var(--shadow-card)}.market-search-submit svg{color:var(--foreground)}.market-search-submit:disabled{opacity:.9;cursor:wait;background:var(--primary);color:var(--foreground)}.market-search-results{position:absolute!important;top:calc(100% + 10px)!important;right:0!important;left:0!important;z-index:9999!important;width:100%;max-height:min(320px,48dvh);overflow-y:auto!important;overflow-x:hidden!important;display:block!important;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-card);box-shadow:var(--shadow-popover);padding:8px;overscroll-behavior:contain}.market-search-results button{width:100%;min-height:68px;height:auto!important;display:grid!important;grid-template-columns:1fr!important;gap:7px;margin:0 0 6px;padding:11px 12px;border-radius:var(--radius-control);text-align:start;cursor:pointer}.market-search-results button:last-of-type{margin-bottom:0}.market-search-results p{margin:0;padding:12px;border-radius:var(--radius-control);text-align:start}.market-search-result-main{display:flex!important;align-items:center;justify-content:space-between;gap:12px;min-width:0;width:100%}.market-search-result-identity{display:flex!important;align-items:center;gap:9px;min-width:0}.market-search-result-identity b{min-width:0}.market-search-results button b{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.market-search-results button small{display:block;width:100%;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
        .market-hero-card{background:color-mix(in srgb,var(--hero-foreground) 10%,transparent);border:1px solid color-mix(in srgb,var(--hero-foreground) 28%,transparent);border-radius:var(--radius-card);padding:18px;display:grid;gap:8px;backdrop-filter:blur(14px)}.market-hero-card span{font-size:12px;color:var(--hero-foreground-muted);font-weight:600}.market-hero-card strong{font-size:42px;color:var(--accent);line-height:1}.market-hero-card p{margin:0;font-size:13px}.market-hero-card em{font-style:normal;color:var(--hero-foreground-muted);font-size:12px;font-weight:600;line-height:1.5}.risk{justify-self:start;border:1px solid transparent;border-radius:var(--radius-pill);padding:6px 10px;font-size:12px;font-weight:700;line-height:1.2}.risk.low{background:var(--success-soft);color:var(--success);border-color:var(--success)}.risk.medium{background:var(--warning-soft);color:var(--foreground-muted);border-color:var(--warning)}.risk.high{background:var(--danger-soft);color:var(--danger);border-color:var(--danger)}
        .market-card-grid,.market-status-grid,.market-decision-grid,.market-tools-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;align-items:stretch}.market-status-grid{grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.market-decision-grid{grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:16px;align-items:start}.market-decision-grid>.market-panel{align-self:start}.market-tools-grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.market-card,.market-panel,.market-disclaimer,.market-notice,.market-stock-header{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-panel);box-shadow:var(--shadow-card)}.market-card{padding:14px;display:grid;gap:8px;min-width:0;height:100%;align-content:start}.market-card-head{display:flex;align-items:flex-start;justify-content:space-between;gap:8px}.market-card-head strong{display:block;font-size:16px}.market-card-head span{display:block;color:var(--foreground-muted);font-size:12px;font-weight:400;margin-top:2px}.market-card-identity{max-width:min(100%,180px)}.market-price{font-size:21px;font-weight:600;color:var(--foreground)}.change{display:inline-flex;align-items:center;gap:4px;width:max-content;border:1px solid transparent;border-radius:var(--radius-pill);padding:5px 9px;font-size:12px;font-weight:700;line-height:1.2;direction:ltr;unicode-bidi:isolate}.change.up{color:var(--success);background:var(--success-soft);border-color:var(--success)}.change.down{color:var(--danger);background:var(--danger-soft);border-color:var(--danger)}.up:not(.change){color:var(--success)}.down:not(.change){color:var(--danger)}.market-empty{grid-column:1/-1;padding:22px;text-align:center;color:var(--foreground-muted);font-weight:600;background:var(--surface);border:1px dashed var(--border);border-radius:var(--radius-card)}.market-empty-detailed{display:grid;gap:10px;justify-items:center;text-align:center}.market-empty-detailed strong{color:var(--foreground);font-size:17px}.market-empty-detailed span{color:var(--foreground-muted);font-size:13px;line-height:1.6}.market-empty-detailed p{margin:0;max-width:680px;color:var(--foreground-muted);line-height:1.8}.market-empty-detailed button{border:0;border-radius:var(--radius-pill);background:var(--surface-muted);color:var(--foreground);padding:10px 16px;font:600 12px var(--font-ui);cursor:pointer}.market-notice{padding:13px 15px;color:var(--danger);background:var(--danger-soft);border-color:var(--danger);font-weight:600;display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap}.market-notice button{border:0;border-radius:var(--radius-pill);background:var(--foreground);color:var(--surface);padding:8px 12px;font:600 12px var(--font-ui);cursor:pointer;white-space:nowrap}.market-notice button:disabled{opacity:.55;cursor:not-allowed}.market-suggestion-chips{flex-basis:100%;display:flex;align-items:center;gap:8px;flex-wrap:wrap}.market-suggestion-chips small{color:var(--foreground-muted);font-size:12px;font-weight:700;line-height:1.4}.market-suggestion-chips button{background:var(--accent-soft);border:1px solid var(--border-strong);color:var(--primary-hover);min-height:34px;padding:0 12px}.market-suggestion-chips button:hover,.market-suggestion-chips button:focus-visible{outline:none;background:var(--accent-soft);border-color:var(--accent);box-shadow:var(--focus-shadow)}.market-notice.success{color:var(--success);background:var(--success-soft);border-color:var(--success)}.market-notice.slow{color:var(--warning);background:var(--warning-soft);border-color:var(--warning)}
        .market-stock-header{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:18px 20px}.market-stock-header h2{margin:8px 0 4px;color:var(--foreground);font-size:28px}.market-stock-header p{margin:0;color:var(--foreground-muted);font-weight:400}.stock-price-block{display:grid;justify-items:end;gap:5px;text-align:end}.stock-price-block strong{font-size:30px;color:var(--foreground);font-weight:700}.stock-price-block small{color:var(--foreground-muted);font-weight:400}.data-badge{display:inline-flex;width:max-content;border-radius:var(--radius-pill);border:1px solid var(--border);background:var(--primary-soft);color:var(--primary-hover);padding:5px 10px;font-size:12px;font-weight:700}.data-badge.delayed{background:var(--warning-soft);border-color:var(--warning);color:var(--warning)}.data-badge.unavailable{background:var(--danger-soft);border-color:var(--danger);color:var(--danger)}
        .market-service{display:flex;align-items:center;gap:9px;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-card);padding:12px 14px;color:var(--foreground-muted);font-weight:600;box-shadow:var(--shadow-card)}.market-service.connected{color:var(--success);background:var(--success-soft);border-color:var(--success)}.market-service.slow,.market-service.not_configured{color:var(--warning);background:var(--warning-soft);border-color:var(--warning)}.market-service.unavailable{color:var(--danger);background:var(--danger-soft);border-color:var(--danger)}
        .market-layout{display:grid;grid-template-columns:minmax(0,1fr) 340px;gap:18px}.market-chart{grid-row:span 2}.market-panel{padding:22px;min-width:0}.market-section-head{display:flex;align-items:flex-start;gap:11px;margin-bottom:16px;color:var(--accent)}.market-section-head span{display:block;color:var(--foreground-muted);font-size:12px;font-weight:600;margin-bottom:5px;line-height:1.4}.market-section-head h2{margin:0;color:var(--foreground);font-size:17px;font-weight:600;line-height:1.35}.market-chart svg{width:100%;height:auto;max-height:300px;display:block}.timeframe-row{display:flex;flex-wrap:wrap;gap:8px;margin:-4px 0 12px}.timeframe-row button{border:1px solid var(--border-strong);background:var(--surface);color:var(--foreground);border-radius:var(--radius-pill);padding:7px 11px;font:600 12px var(--font-ui);cursor:pointer}.timeframe-row button[aria-pressed="true"],.timeframe-row button:hover,.timeframe-row button:focus-visible{background:var(--primary);border-color:transparent;color:var(--primary-foreground);outline:none;box-shadow:var(--focus-shadow)}.price-history-chart{position:relative;min-height:320px;border-radius:var(--radius-card);border:1px solid var(--border);background:var(--surface);overflow:hidden;padding:16px;display:grid;align-content:stretch}.price-history-chart svg{width:100%;height:260px;max-height:none;display:block;overflow:visible}.price-chart-values{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:8px}.price-chart-values span{color:var(--foreground-muted);font-size:12px;font-weight:600;line-height:1.4}.price-chart-values strong{color:var(--foreground);font-size:20px;font-weight:700;line-height:1.2}.price-chart-grid-line{stroke:var(--chart-grid);stroke-width:1}.price-chart-line-path{fill:none;stroke-width:4;stroke-linecap:round;stroke-linejoin:round;filter:none}.price-chart-area-path{pointer-events:none}.price-chart-last-dot{fill:var(--foreground);stroke:var(--accent);stroke-width:4}.price-chart-axis{display:flex;align-items:center;justify-content:space-between;gap:8px;color:var(--foreground-muted);font-size:12px;font-weight:600}.price-chart-state{position:absolute;inset:16px;display:grid;place-items:center;gap:10px;text-align:center;border:1px dashed var(--border);background:var(--surface);backdrop-filter:blur(10px);border-radius:var(--radius-card);color:var(--foreground-muted);padding:22px;line-height:1.8}.price-chart-state svg{width:18px;height:18px;color:var(--warning)}.price-chart-state strong{color:var(--foreground);font-size:14px;font-weight:700;max-width:520px}.price-chart-loading{position:absolute;inset:16px;display:flex;align-items:center;justify-content:center;gap:10px;border-radius:var(--radius-card);background:var(--surface);backdrop-filter:blur(8px);color:var(--foreground);font-size:13px;font-weight:700;line-height:1.6}.price-chart-loading span{width:10px;height:10px;border-radius:var(--radius-pill);background:var(--accent);box-shadow:var(--shadow-card);animation:marketPulse 1.1s ease-in-out infinite}.market-chart-meta{display:flex;flex-wrap:wrap;gap:8px;margin-top:10px;color:var(--foreground-muted);font-size:12px;font-weight:600}.market-chart-meta span{border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-pill);padding:6px 9px}.market-chart-meta b{color:var(--foreground)}.market-stat-row{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin-top:14px}.metric{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-card);padding:13px;display:grid;gap:7px;min-width:0;align-content:start}.metric span{font-size:12px;color:var(--foreground-muted);font-weight:600;line-height:1.45}.metric strong{font-size:15px;color:var(--foreground);display:flex;align-items:center;gap:6px;min-width:0;overflow-wrap:anywhere;line-height:1.45}.indicator-list,.scenario-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.portfolio-card{display:grid;gap:14px;align-content:start;padding:18px}.portfolio-card .market-section-head{margin-bottom:0}.portfolio-metric-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.portfolio-metric-grid .metric{border-radius:var(--radius-control);padding:11px 12px;gap:5px}.portfolio-metric-grid .metric span{font-size:12px;line-height:1.35}.portfolio-metric-grid .metric strong{font-size:14px;line-height:1.35}.fundamentals-empty{display:grid;gap:9px;background:var(--surface);border:1px dashed var(--border);border-radius:var(--radius-card);padding:16px;color:var(--foreground-muted);line-height:1.8}.fundamentals-empty svg{color:var(--accent)}.fundamentals-empty strong{color:var(--foreground);font-size:15px;font-weight:700;line-height:1.45}.fundamentals-empty p{margin:0;color:var(--foreground-muted);font-weight:400}.fundamentals-empty small{width:max-content;max-width:100%;border:1px solid var(--border);background:var(--accent-soft);color:var(--primary-hover);border-radius:var(--radius-pill);padding:5px 9px;font-weight:700;line-height:1.35}.asset-profile-card{margin:16px 0;display:grid;gap:0}.asset-profile-body{display:grid;gap:14px}.asset-profile-header{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-card);padding:15px}.asset-profile-header strong{display:block;color:var(--foreground);font-size:18px;font-weight:700;line-height:1.35}.asset-profile-header p,.asset-profile-section p{margin:6px 0 0;color:var(--foreground-muted);font-size:13px;font-weight:400;line-height:1.8}.asset-profile-badges{display:flex;flex-wrap:wrap;gap:7px;justify-content:flex-end}.asset-profile-badges span{border:1px solid var(--border);background:var(--surface);color:var(--foreground);border-radius:var(--radius-pill);padding:6px 9px;font-size:12px;font-weight:700;line-height:1.2}.asset-profile-section{display:grid;gap:10px;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-card);padding:14px}.asset-profile-section h3{margin:0;display:flex;align-items:center;gap:7px;color:var(--foreground);font-size:14px;font-weight:700;line-height:1.4}.asset-profile-metrics{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.asset-profile-metric{display:grid;gap:5px;min-width:0;border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-control);padding:10px}.asset-profile-metric span{color:var(--foreground-muted);font-size:12px;font-weight:600;line-height:1.35}.asset-profile-metric strong{color:var(--foreground);font-size:13px;font-weight:700;line-height:1.45;overflow-wrap:anywhere}.asset-profile-link{min-height:58px;display:flex;align-items:center;justify-content:center;gap:7px;border:1px solid var(--border);background:var(--accent-soft);border-radius:var(--radius-control);color:var(--primary-hover);font-size:12px;font-weight:700;text-decoration:none}.asset-profile-link:hover,.asset-profile-link:focus-visible{outline:none;border-color:var(--accent);box-shadow:var(--focus-shadow)}.asset-profile-holdings{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.asset-profile-holdings span{display:flex;align-items:center;justify-content:space-between;gap:8px;border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-control);padding:9px 10px;min-width:0}.asset-profile-holdings b{color:var(--foreground);font-size:12px;font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.asset-profile-holdings em{color:var(--accent);font-size:12px;font-weight:700;font-style:normal;white-space:nowrap}.asset-profile-limitations ul{margin:0;padding-inline-start:18px;color:var(--foreground-muted);font-size:12px;font-weight:400;line-height:1.8}.asset-profile-footer{display:flex;flex-wrap:wrap;gap:8px;color:var(--foreground-muted);font-size:12px;font-weight:600}.asset-profile-footer span{border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-pill);padding:6px 9px}.asset-profile-state{display:flex;align-items:center;gap:9px;border:1px dashed var(--border);background:var(--surface);border-radius:var(--radius-card);padding:14px;color:var(--foreground-muted);font-size:13px;font-weight:600;line-height:1.6}.asset-profile-state.error{border-color:var(--danger);color:var(--danger)}.asset-profile-pulse{width:10px;height:10px;border-radius:var(--radius-pill);background:var(--accent);box-shadow:var(--shadow-card);animation:marketPulse 1.1s ease-in-out infinite}.decision-body{display:grid;gap:13px}.decision-body b{font-size:25px;font-weight:600;color:var(--foreground);line-height:1.25}.decision-body p,.decision-body small,.decision-body strong{margin:0;color:var(--foreground-muted);line-height:1.9;font-weight:400;overflow-wrap:anywhere}.decision-body small{font-size:12px;color:var(--foreground-muted)}.decision-body strong{color:var(--foreground-muted)}.ai-summary-body{align-content:start}.ai-summary-intro{border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-control);padding:10px 12px;color:var(--foreground)!important;font-size:13px;line-height:1.75}.ai-summary-section{display:grid;gap:5px;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-card);padding:12px}.ai-summary-section b{font-size:13px;color:var(--foreground);font-weight:700}.ai-summary-section p{font-size:13px;line-height:1.75}.ai-summary-section ul{margin:0;padding-inline-start:18px;color:var(--foreground-muted);font-size:13px;font-weight:400;line-height:1.75}.ai-summary-loading{display:flex;align-items:center;gap:9px;color:var(--foreground-muted);font-size:13px;font-weight:700}.ai-summary-loading span{width:10px;height:10px;border-radius:var(--radius-pill);background:var(--accent);box-shadow:var(--shadow-card);animation:marketPulse 1.1s ease-in-out infinite}@keyframes marketPulse{50%{transform:scale(.72);opacity:.55}}.ai-summary-actions{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap}.ai-summary-actions small{flex:1;min-width:180px}.ai-regenerate{margin:0;width:max-content;background:var(--primary);color:var(--foreground)}.risk-score{display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:10px;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-control);padding:10px 12px}.risk-score span,.risk-score strong{font-size:12px!important;color:var(--foreground)!important;font-weight:700!important;line-height:1.3!important}.risk-score i{height:9px;border-radius:var(--radius-pill);background:var(--primary-soft);overflow:hidden}.risk-score i b{display:block;height:100%;border-radius:inherit;background:var(--danger-soft)}.decision.ok{border-color:var(--success)}.decision.warn{border-color:var(--warning)}.decision.danger{border-color:var(--danger)}.levels-strip{margin-top:16px;display:grid;gap:10px;color:var(--foreground-muted);font-size:12px;font-weight:600;line-height:1.6}.levels-strip i{position:relative;height:10px;border-radius:var(--radius-pill);background:var(--danger-soft);display:block}.levels-strip b{position:absolute;top:-4px;width:4px;height:18px;border-radius:var(--radius-pill);background:var(--foreground)}.levels-strip b.current{width:10px;height:10px;top:0;transform:translateX(-50%);background:var(--surface);border:2px solid var(--foreground)}.tool-input,.alert-form{display:grid;gap:10px}.tool-input span{font-size:12px;font-weight:600;color:var(--foreground-muted);line-height:1.5}.tool-input input,.alert-form input,.alert-form select{width:100%;min-width:0;border:1px solid var(--border-strong);border-radius:var(--radius-control);background:var(--surface);padding:12px 13px;font:600 13px var(--font-ui);outline:0;line-height:1.5}.market-currency-input{min-height:52px;display:flex;align-items:center;gap:9px;border:1px solid var(--border-strong);border-radius:var(--radius-control);background:var(--surface);padding:0 9px;transition:border-color .18s ease,box-shadow .18s ease;position:relative}.market-currency-input:focus-within{border-color:var(--accent);box-shadow:var(--focus-shadow)}.tool-input .market-currency-input input{border:0;background:transparent;padding:0;box-shadow:none;flex:1;min-width:0}.tool-input .market-currency-input input::placeholder{color:var(--foreground-muted);opacity:1}.market-scenario-currency{position:relative;flex:0 0 auto;display:flex;align-items:center;gap:7px}.market-scenario-currency>span{color:var(--foreground-muted);font-size:12px;font-weight:700;line-height:1.2}.market-scenario-currency-trigger{min-width:62px;height:36px;border-radius:var(--radius-pill);border:1px solid var(--success);background:var(--surface);color:var(--success);padding:0 10px;display:inline-flex;align-items:center;justify-content:center;gap:6px;font:700 12px var(--font-ui);cursor:pointer;white-space:nowrap}.market-scenario-currency-trigger:focus-visible{outline:none;border-color:var(--accent);box-shadow:var(--focus-shadow)}.market-scenario-currency-trigger b{direction:rtl;unicode-bidi:isolate;font-size:12px}.market-scenario-currency-trigger small{font-size:12px;line-height:1}.market-scenario-currency-menu{position:absolute;inset-inline-end:0;top:calc(100% + 8px);z-index:80;width:150px;max-height:260px;overflow:auto;border:1px solid var(--border);border-radius:var(--radius-control);background:var(--surface);box-shadow:var(--shadow-popover);padding:6px;display:grid;gap:5px}.market-scenario-currency-menu button{height:var(--control-h-sm);border:1px solid transparent;border-radius:var(--radius-control);background:transparent;color:var(--foreground);display:flex;align-items:center;justify-content:space-between;gap:10px;padding:0 10px;font:600 12px var(--font-ui);cursor:pointer}.market-scenario-currency-menu button:hover,.market-scenario-currency-menu button:focus-visible,.market-scenario-currency-menu button[aria-selected="true"]{outline:none;background:var(--primary);border-color:var(--focus-ring)}.market-scenario-currency-menu button span{color:var(--foreground);font-size:13px}.market-scenario-currency-menu button b{direction:ltr;unicode-bidi:isolate;color:var(--foreground-muted);font-size:12px}.market-field-error{display:block;color:var(--danger);font-size:12px;font-weight:600;line-height:1.55}.alert-form{grid-template-columns:minmax(0,1fr) 110px auto;align-items:stretch}.alert-form button,.inline-action,.report-button{border:0;border-radius:var(--radius-control);background:var(--foreground);color:var(--surface);padding:12px 14px;font:600 12px var(--font-ui);display:inline-flex;align-items:center;justify-content:center;gap:8px;cursor:pointer;line-height:1.45}.inline-action{margin-bottom:14px;background:var(--surface-muted);color:var(--foreground)}.inline-action:disabled{opacity:.58;cursor:default}.saved-alerts,.asset-report{display:grid;gap:10px;margin-top:14px}.saved-alerts span,.asset-report p{margin:0;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-control);padding:10px 11px;color:var(--foreground-muted);font-size:12px;font-weight:600;line-height:1.65}.saved-alerts span{display:flex;align-items:center;justify-content:space-between;gap:9px}.saved-alerts span b{min-width:0;overflow-wrap:anywhere}.saved-alerts button{border:0;background:transparent;color:var(--foreground-muted);cursor:pointer;display:inline-flex;padding:2px}.asset-report small{color:var(--foreground-muted);line-height:1.75;font-weight:400;display:block}
        .ai-summary-compact{display:grid;gap:10px;align-content:start}.ai-summary-compact small{color:var(--foreground-muted);font-size:12px;font-weight:400;line-height:1.7}
        .economic-calendar-panel{display:grid;gap:16px;overflow:hidden}.economic-calendar-empty{display:grid;grid-template-columns:auto minmax(0,1fr);gap:16px;border:1px solid var(--warning);background:var(--warning-soft);border-radius:var(--radius-panel);padding:20px;box-shadow:var(--shadow-card)}.economic-calendar-empty-icon{width:54px;height:54px;border-radius:var(--radius-card);display:grid;place-items:center;background:var(--warning-soft);border:1px solid var(--warning);color:var(--warning)}.economic-calendar-empty small{display:block;color:var(--warning);font-size:12px;font-weight:700;line-height:1.4;margin-bottom:5px}.economic-calendar-empty strong{display:block;color:var(--foreground);font-size:clamp(18px,2vw,24px);font-weight:700;line-height:1.35}.economic-calendar-empty p{margin:8px 0 0;max-width:760px;color:var(--foreground-muted);font-size:14px;font-weight:400;line-height:1.9}.economic-calendar-empty em{display:inline-flex;width:max-content;max-width:100%;margin-top:12px;border:1px solid var(--warning);background:var(--warning-soft);color:var(--primary-hover);border-radius:var(--radius-pill);padding:7px 11px;font-style:normal;font-size:12px;font-weight:700;line-height:1.4}.economic-calendar-chips{display:flex;flex-wrap:wrap;gap:8px;margin-top:13px}.economic-calendar-chips span{border:1px solid var(--warning);background:var(--surface);color:var(--warning);border-radius:var(--radius-pill);padding:7px 10px;font-size:12px;font-weight:700;line-height:1.2}.economic-calendar-next{display:flex;align-items:center;justify-content:space-between;gap:14px;border:1px solid var(--border);background:var(--accent-soft);border-radius:var(--radius-panel);padding:16px;box-shadow:var(--shadow-card)}.economic-calendar-next span{display:block;color:var(--foreground-muted);font-size:12px;font-weight:700;margin-bottom:5px}.economic-calendar-next strong{display:block;color:var(--foreground);font-size:18px;font-weight:700;line-height:1.4}.economic-calendar-next-meta{display:flex;align-items:center;gap:8px;flex-wrap:wrap;justify-content:flex-end}.economic-calendar-next-meta b,.economic-calendar-next-meta small{border:1px solid var(--border);background:var(--accent-soft);color:var(--primary-hover);border-radius:var(--radius-pill);padding:7px 10px;font-size:12px;font-weight:700;line-height:1.2}.economic-calendar-filters{width:100%;max-width:100%;display:flex;gap:8px;overflow-x:auto;padding:2px 2px 8px;scrollbar-width:none;-webkit-overflow-scrolling:touch}.economic-calendar-filters::-webkit-scrollbar{display:none}.economic-calendar-filters button{flex:0 0 auto;min-height:40px;border:1px solid var(--border-strong);border-radius:var(--radius-pill);background:var(--surface);color:var(--foreground-muted);padding:0 14px;font:700 12px var(--font-ui);cursor:pointer;white-space:nowrap}.economic-calendar-filters button[aria-pressed="true"],.economic-calendar-filters button:hover,.economic-calendar-filters button:focus-visible{outline:none;background:var(--primary);border-color:transparent;color:var(--primary-foreground);box-shadow:var(--focus-shadow)}.economic-calendar-list{display:grid;gap:12px}.economic-calendar-event{display:grid;gap:14px;border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-panel);padding:15px;box-shadow:var(--shadow-card)}.economic-calendar-event-main{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.economic-calendar-event-main b{color:var(--foreground);font-size:15px;font-weight:700;line-height:1.5}.economic-calendar-event-main span{flex:0 0 auto;color:var(--foreground-muted);font-size:12px;font-weight:700;border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-pill);padding:6px 9px}.economic-calendar-event-metrics{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.economic-calendar-metric{display:grid;gap:5px;min-width:0;border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-control);padding:10px}.economic-calendar-metric small{color:var(--foreground-muted);font-size:12px;font-weight:600;line-height:1.35}.economic-calendar-metric b{color:var(--foreground);font-size:12px;font-weight:700;line-height:1.45;overflow-wrap:anywhere}
        .market-bottom-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.market-copy,.market-muted{margin:0;color:var(--foreground-muted);line-height:1.8;font-size:13px;font-weight:400}.market-muted{margin-top:12px;color:var(--foreground-muted);font-size:12px}.watchlist{display:flex;flex-wrap:wrap;gap:8px}.watchlist>span,.watchlist>button{border-radius:var(--radius-pill);background:var(--surface);border:1px solid var(--border-strong);padding:7px 11px;color:var(--foreground-muted);font-weight:600;font-size:12px;display:inline-flex;align-items:center;gap:6px}.watchlist button{border:0;background:transparent;color:inherit;font:inherit;cursor:pointer;padding:0}.market-card-identity{min-width:0;max-width:100%}.saved-alerts b{display:inline-flex;align-items:center;gap:8px;min-width:0}.saved-alerts b>span:last-child{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.compare-bars{display:grid;gap:10px}.compare-bars div{display:grid;grid-template-columns:112px minmax(0,1fr) 54px;gap:8px;align-items:center}.compare-bars span,.compare-bars b{font-size:12px;font-weight:600;color:var(--foreground-muted)}.compare-bars div i{height:9px;border-radius:var(--radius-pill);background:var(--primary);display:block}.compare-table{margin-top:14px;overflow-x:auto;display:grid;gap:7px}.compare-table>div{display:grid;grid-template-columns:minmax(118px,1.25fr) 90px 70px 52px 76px;gap:7px;min-width:456px}.compare-table b,.compare-table span{font-size:12px;font-weight:600;color:var(--foreground-muted)}.compare-table b{color:var(--foreground-muted);max-width:100%}
        .trader-dashboard{display:grid;gap:16px}.tool-tabs,.symbol-chip-row,.overlap-row{display:flex;flex-wrap:wrap;gap:8px}.tool-tabs button,.symbol-chip-row button{min-height:38px;border:1px solid var(--border-strong);border-radius:var(--radius-pill);background:var(--surface);color:var(--foreground-muted);padding:0 13px;font:600 12px var(--font-ui);cursor:pointer}.tool-tabs button[aria-pressed="true"],.tool-tabs button:hover,.tool-tabs button:focus-visible,.symbol-chip-row button[aria-pressed="true"],.symbol-chip-row button:hover,.symbol-chip-row button:focus-visible{outline:none;background:var(--primary);border-color:var(--focus-ring);color:var(--foreground);box-shadow:var(--focus-shadow)}.trader-tool-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.trader-tool-switcher-shell{display:grid;grid-template-columns:36px minmax(0,1fr) 36px;gap:8px;align-items:center;border:1px solid var(--border);border-radius:var(--radius-panel);background:var(--surface);padding:6px;min-width:0}.trader-switcher-arrow{width:36px;height:36px;border-radius:var(--radius-control);border:1px solid var(--border);background:var(--surface);color:var(--primary-hover);display:grid;place-items:center;cursor:pointer;flex:0 0 auto;transition:background .18s ease,border-color .18s ease,transform .18s ease}.trader-switcher-arrow:hover{background:var(--accent-soft);border-color:var(--border);transform:scale(1.06)}.trader-switcher-arrow:disabled{opacity:.38;cursor:default;transform:none}.trader-tool-switcher{display:flex;gap:6px;overflow:hidden;scroll-snap-type:x mandatory;-webkit-overflow-scrolling:touch;scrollbar-width:none;min-width:0}.trader-tool-switcher::-webkit-scrollbar{display:none}.trader-tool-switcher>button{flex:1;min-width:0;display:flex;align-items:center;gap:10px;border:1px solid var(--border-strong);border-radius:var(--radius-card);background:var(--surface);color:var(--foreground-muted);padding:10px 14px;font:600 13px var(--font-ui);cursor:pointer;transition:all .2s ease;text-align:start;white-space:nowrap;scroll-snap-align:start;min-height:56px}.trader-tool-switcher>button:hover{background:var(--accent-soft);color:var(--foreground);border-color:var(--border-strong);transform:translateY(-1px)}.trader-tool-switcher>button[aria-selected='true']{background:var(--primary);border-color:transparent;color:var(--primary-foreground);box-shadow:var(--shadow-card);transform:translateY(-2px)}.trader-tool-switcher>button>span:not(.trader-switcher-icon){display:grid;gap:2px;min-width:0}.trader-tool-switcher>button>span:not(.trader-switcher-icon)>strong{display:block;font-size:12px;font-weight:700;line-height:1.35;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.trader-tool-switcher>button>span:not(.trader-switcher-icon)>small{display:block;font-size:12px;font-weight:400;line-height:1.4;opacity:.78;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.trader-tool-switcher>button[aria-selected='true']>span:not(.trader-switcher-icon)>small{opacity:.88}.trader-switcher-icon{width:34px;height:34px;border-radius:var(--radius-control);display:grid;place-items:center;flex:0 0 auto;background:var(--accent-soft);color:var(--primary-hover);border:1px solid var(--border);transition:background .18s ease}.trader-tool-switcher>button[aria-selected='true'] .trader-switcher-icon{background:var(--primary);color:var(--primary-foreground);border-color:var(--border-strong)}.trader-tool-input-card .trader-tool-card-head,.trader-tool-card-head{display:flex;align-items:flex-start;gap:12px;min-width:0;border-bottom:1px solid var(--border-strong);padding-bottom:12px;margin-bottom:4px}.trader-tool-card-head>span{width:42px;height:42px;border-radius:var(--radius-card);display:grid;place-items:center;flex:0 0 auto;background:var(--surface);color:var(--foreground);box-shadow:var(--shadow-card)}.trader-tool-card-head>div{display:grid;gap:3px;min-width:0;flex:1}.trader-tool-card-head h3{margin:0;color:var(--foreground);font-size:16px;font-weight:700;line-height:1.35}.trader-tool-card-head p{margin:0;color:var(--foreground-muted);font-size:12px;font-weight:400;line-height:1.6}.tool-reset{align-self:flex-start;border:1px solid var(--border);border-radius:var(--radius-pill);background:var(--primary-soft);color:var(--primary-hover);padding:6px 14px;font:700 12px var(--font-ui);cursor:pointer;white-space:nowrap;transition:background .18s ease,border-color .18s ease}.tool-reset:hover{background:var(--primary-soft);border-color:var(--border)}.tool-advanced{grid-column:1/-1;min-width:0}.tool-advanced>summary{color:var(--primary-hover);font-size:12px;font-weight:700;cursor:pointer;padding:6px 0;list-style:none}@media(max-width:720px){.trader-tool-switcher-shell{grid-template-columns:32px minmax(0,1fr) 32px;gap:4px;border-radius:var(--radius-card);padding:4px}.trader-tool-switcher>button{min-height:48px;padding:8px 10px}.trader-tool-switcher>button>span:not(.trader-switcher-icon)>small{display:none}.trader-switcher-icon{width:28px;height:28px;border-radius:var(--radius-sm)}.trader-switcher-arrow{width:32px;height:32px;border-radius:var(--radius-sm)}}.trader-tool-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-card);padding:16px;display:grid;gap:14px;min-width:0}.trader-tool-card.compact{align-content:start}.trader-tool-card h3{margin:0;color:var(--foreground);font-size:16px;font-weight:700;line-height:1.45}.trader-form-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.tool-input select{width:100%;min-width:0;border:1px solid var(--border-strong);border-radius:var(--radius-control);background:var(--surface);padding:12px 13px;font:600 13px var(--font-ui);outline:0;color:var(--foreground)}.tool-result-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.tool-warning{margin:0;border:1px solid var(--warning);background:var(--warning-soft);color:var(--warning);border-radius:var(--radius-control);padding:10px 12px;font-size:12px;font-weight:600;line-height:1.7}.trader-table-wrap{overflow-x:auto;border:1px solid var(--border);border-radius:var(--radius-card)}.trader-table{display:grid;min-width:780px}.trader-table>div{display:grid;grid-template-columns:90px minmax(160px,1fr) 110px 90px 90px 90px 90px;gap:10px;padding:11px 12px;border-bottom:1px solid var(--border);align-items:center}.trader-table>div:last-child{border-bottom:0}.trader-table b{color:var(--foreground-muted);font-size:12px;font-weight:700}.trader-table span{color:var(--foreground);font-size:12px;font-weight:600;overflow-wrap:anywhere}.trader-empty-state{display:grid;gap:9px;justify-items:start;border:1px dashed var(--border);background:var(--surface);border-radius:var(--radius-card);padding:18px;color:var(--foreground-muted)}.trader-empty-state svg{color:var(--accent)}.trader-empty-state strong{color:var(--foreground);font-size:15px;font-weight:700}.trader-empty-state p{margin:0;color:var(--foreground-muted);font-size:13px;font-weight:400;line-height:1.8}.session-timeline{position:relative;height:48px;border-radius:var(--radius-pill);background:var(--accent-soft);border:1px solid var(--border);overflow:hidden}.session-timeline span{position:absolute;top:7px;bottom:7px;border-radius:var(--radius-pill);background:var(--surface-muted);color:var(--foreground-muted);display:grid;place-items:center;font-size:12px;font-weight:700;min-width:54px}.session-timeline span.open{background:var(--surface-muted);color:var(--foreground)}.sessions-grid{margin-top:14px}.session-badge{width:max-content;border-radius:var(--radius-pill);padding:6px 10px;background:var(--surface-muted);color:var(--foreground-muted);font-size:12px}.session-badge.open{background:var(--accent-soft);color:var(--primary-hover)}.overlap-row{margin-top:14px}.overlap-row span{border:1px solid var(--border);background:var(--surface);color:var(--foreground-muted);border-radius:var(--radius-pill);padding:7px 10px;font-size:12px;font-weight:600}.overlap-row span.active{background:var(--accent-soft);border-color:var(--border);color:var(--primary-hover)}
        .technical-analysis-panel{display:grid;gap:16px;overflow:hidden}.technical-selector-shell{display:grid;gap:13px;border:1px solid var(--border-strong);background:var(--surface);border-radius:var(--radius-panel);padding:14px;min-width:0}.technical-search{min-height:48px;display:flex;align-items:center;gap:10px;border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-card);padding:0 13px;color:var(--foreground-muted);min-width:0}.technical-search:focus-within{border-color:var(--accent);box-shadow:var(--focus-shadow)}.technical-search input{flex:1;min-width:0;border:0;outline:0;background:transparent;color:var(--foreground);font:600 13px var(--font-ui);line-height:1.4}.technical-search input::placeholder{color:var(--foreground-muted);opacity:1}.technical-search button{width:28px;height:28px;border:0;border-radius:var(--radius-pill);background:var(--surface-muted);color:var(--foreground-muted);font:700 15px var(--font-ui),sans-serif;cursor:pointer}.technical-category-row,.technical-symbol-row{width:100%;max-width:100%;display:flex;flex-wrap:nowrap;gap:9px;overflow-x:auto;overflow-y:hidden;padding:2px 2px 8px;scrollbar-width:none;-webkit-overflow-scrolling:touch}.technical-category-row::-webkit-scrollbar,.technical-symbol-row::-webkit-scrollbar{display:none}.technical-category-row button{flex:0 0 auto;min-height:40px;border:1px solid var(--border-strong);border-radius:var(--radius-pill);background:var(--surface);color:var(--foreground-muted);padding:0 15px;font:700 12px var(--font-ui);cursor:pointer;white-space:nowrap}.technical-category-row button[aria-pressed="true"],.technical-category-row button:hover,.technical-category-row button:focus-visible{outline:none;background:var(--primary);border-color:transparent;color:var(--primary-foreground);box-shadow:var(--focus-shadow)}.technical-symbol-pill{flex:0 0 auto;display:flex;align-items:center;gap:4px;min-height:42px;border:1px solid var(--border);border-radius:var(--radius-pill);background:var(--surface);padding:3px;color:var(--foreground);box-shadow:var(--shadow-card)}.technical-symbol-pill[data-active="true"]{border-color:transparent;background:var(--surface-muted);color:var(--primary-foreground);box-shadow:var(--shadow-card)}.technical-symbol-main,.technical-symbol-favorite{border:0;background:transparent;color:inherit;cursor:pointer;display:inline-flex;align-items:center;justify-content:center}.technical-symbol-main{min-height:34px;padding:0 10px;font:700 13px var(--font-ui);letter-spacing:.01em}.technical-symbol-main span{unicode-bidi:isolate}.technical-symbol-favorite{width:32px;height:32px;border-radius:var(--radius-pill);color:inherit;opacity:.78}.technical-symbol-favorite:hover,.technical-symbol-favorite:focus-visible{outline:none;background:var(--surface);opacity:1}.technical-symbol-pill:not([data-active="true"]) .technical-symbol-favorite:hover,.technical-symbol-pill:not([data-active="true"]) .technical-symbol-favorite:focus-visible{background:var(--accent-soft);color:var(--primary-hover)}.technical-favorites{display:grid;gap:6px;min-width:0}.technical-favorites>span{color:var(--foreground-muted);font-size:12px;font-weight:700;line-height:1.4}.technical-symbol-row.compact{padding-bottom:2px}.technical-no-results{flex:0 0 auto;border:1px dashed var(--border);border-radius:var(--radius-pill);background:var(--surface);color:var(--foreground-muted);padding:10px 13px;font-size:12px;font-weight:600}.technical-selected-summary{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.technical-result-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.market-status-grid .metric{border-radius:var(--radius-card);background:var(--surface);border-color:var(--border)}
        .market-panel.trader-dashboard{width:100%;max-width:100%;overflow:hidden;padding:clamp(16px,4vw,28px);border-radius:var(--radius-panel);background:var(--surface);display:grid;gap:18px;box-sizing:border-box}.trader-dashboard *{box-sizing:border-box}.trader-dashboard-head{margin:0;align-items:center;border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-panel);padding:14px;min-width:0}.trader-dashboard-head>div{min-width:0}.trader-dashboard-head h2{font-size:clamp(24px,5vw,34px);line-height:1.15}.trader-head-icon,.trader-tool-card-head span{width:42px;height:42px;border-radius:var(--radius-card);display:grid;place-items:center;flex:0 0 auto;background:var(--surface);color:var(--accent);border:1px solid var(--border)}.tool-tabs{width:100%;max-width:100%;overflow-x:auto;overflow-y:hidden;display:flex;flex-wrap:nowrap;gap:8px;padding:2px 2px 8px;scrollbar-width:none;-webkit-overflow-scrolling:touch}.tool-tabs::-webkit-scrollbar{display:none}.tool-tabs button{flex:0 0 auto;white-space:nowrap;min-height:42px;border-radius:var(--radius-card);padding:0 14px;font-size:13px;max-width:none}.trader-tool-grid{grid-template-columns:1fr;gap:16px;min-width:0}.trader-tool-card{width:100%;max-width:100%;overflow:hidden;border-radius:var(--radius-panel);padding:18px;gap:16px;background:var(--surface);border-color:var(--border);box-shadow:var(--shadow-card)}.trader-tool-card-head{display:flex;align-items:center;gap:10px;min-width:0}.trader-tool-card-head h3{font-size:clamp(18px,4vw,22px);line-height:1.35;min-width:0;overflow-wrap:anywhere}.trader-form-grid{grid-template-columns:1fr;gap:14px;min-width:0}.tool-input{width:100%;max-width:100%;display:grid!important;grid-template-columns:1fr!important;gap:8px;min-width:0;align-items:start}.tool-input span{display:block;color:var(--foreground);font-size:13px;font-weight:700;line-height:1.55;text-align:start}.tool-input input,.tool-input select{width:100%;max-width:100%;min-width:0;height:var(--control-h-lg);border:1px solid var(--border-strong);border-radius:var(--radius-card);background:var(--surface);color:var(--foreground);padding:0 14px;font:600 16px var(--font-ui);outline:0;box-shadow:none}.tool-input input:focus,.tool-input select:focus{border-color:var(--accent);box-shadow:var(--shadow-card)}.tool-input input[dir="ltr"]{text-align:left;direction:ltr;unicode-bidi:isolate}.tool-results{display:grid;gap:10px;border-top:1px solid var(--border);padding-top:14px;min-width:0}.tool-results-title{color:var(--foreground);font-size:14px;font-weight:700;line-height:1.4}.tool-result-grid{grid-template-columns:1fr;gap:10px;min-width:0}.tool-result-card{display:grid;gap:5px;min-width:0;border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-card);padding:13px 14px}.tool-result-card span{color:var(--foreground-muted);font-size:12px;font-weight:600;line-height:1.45;text-align:start}.tool-result-card b{color:var(--foreground);font-size:18px;font-weight:700;line-height:1.3;overflow-wrap:anywhere;text-align:start}.tool-warning{display:block;margin:0;border-radius:var(--radius-card);padding:12px 14px;font-size:13px;line-height:1.75}@media(min-width:640px){.tool-result-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(min-width:760px){.trader-form-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(min-width:1180px){.trader-tool-grid{grid-template-columns:repeat(3,minmax(0,1fr))}}[dir="rtl"] .trader-dashboard,[dir="rtl"] .trader-tool-card,[dir="rtl"] .tool-input span,[dir="rtl"] .tool-result-card span{text-align:right}[dir="rtl"] .tool-result-card b{text-align:left}[dir="ltr"] .trader-dashboard,[dir="ltr"] .trader-tool-card,[dir="ltr"] .tool-input span,[dir="ltr"] .tool-result-card span,[dir="ltr"] .tool-result-card b{text-align:left}
        .trader-dashboard-head{justify-content:space-between;gap:14px}.trader-dashboard-head small{margin-inline-start:auto;border:1px solid var(--border);background:var(--surface);color:var(--primary-hover);border-radius:var(--radius-pill);padding:7px 10px;font-size:12px;font-weight:700;line-height:1.3;white-space:nowrap}.trader-tool-grid{display:grid;grid-template-columns:repeat(12,minmax(0,1fr));gap:18px}.trader-tool-card{grid-column:span 4;border-radius:var(--radius-panel);padding:20px;background:var(--surface);border-color:var(--border);box-shadow:var(--shadow-card);align-content:start}.trader-tool-card-head{align-items:flex-start;justify-content:space-between;gap:12px;border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-card);padding:12px}.trader-tool-card-head>div{display:grid;gap:5px;min-width:0}.trader-tool-card-head h3{margin:0;color:var(--foreground);font-size:clamp(17px,1.4vw,21px);font-weight:700;line-height:1.35}.trader-tool-card-head p{margin:0;color:var(--foreground-muted);font-size:12px;font-weight:400;line-height:1.7}.tool-reset{min-height:36px;border:1px solid var(--border);border-radius:var(--radius-pill);background:var(--surface);color:var(--primary-hover);padding:0 12px;font:700 12px var(--font-ui);cursor:pointer;white-space:nowrap}.tool-reset:hover,.tool-reset:focus-visible{outline:none;border-color:var(--accent);box-shadow:var(--focus-shadow);background:var(--accent-soft)}.tool-input{gap:8px}.tool-input span{color:var(--foreground);font-size:12px;font-weight:700;letter-spacing:0}.tool-input small{color:var(--foreground-muted);font-size:12px;font-weight:400;line-height:1.6}.tool-input-shell,.market-currency-input{width:100%;min-height:56px;display:flex;align-items:center;gap:10px;border:1px solid var(--border-strong);border-radius:var(--radius-card);background:var(--surface);padding:0 10px;box-shadow:var(--shadow-card);transition:border-color .18s ease,box-shadow .18s ease,background .18s ease}.tool-input-shell:focus-within,.market-currency-input:focus-within{border-color:var(--accent);box-shadow:var(--focus-shadow)}.tool-input-shell em,.market-input-affix{flex:0 0 auto;min-width:48px;text-align:center;border:1px solid var(--border-strong);background:var(--surface);color:var(--primary-hover);border-radius:var(--radius-control);padding:8px 10px;font-style:normal;font-size:12px;font-weight:700;line-height:1.2;white-space:nowrap}.tool-input .tool-input-shell input,.tool-input .tool-input-shell select,.tool-input .market-currency-input input{height:var(--control-h-lg)!important;width:100%;min-width:0;border:0!important;background:transparent!important;color:var(--foreground);padding:0!important;box-shadow:none!important;font:700 16px var(--font-ui);outline:0}.tool-input .tool-input-shell input[dir="ltr"],.tool-input .market-currency-input input{text-align:left;direction:ltr;unicode-bidi:isolate}.tool-input-shell.select{padding-inline-end:12px}.tool-input-shell.select select{cursor:pointer}.market-scenario-currency{flex:0 0 auto}.market-scenario-currency>span{display:none}.market-scenario-currency-trigger{height:38px;min-width:54px;border-radius:var(--radius-control)}.tool-results{border:1px solid var(--border);border-radius:var(--radius-panel);background:var(--surface);padding:14px;display:grid;gap:12px}.tool-results-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap}.tool-results-head small{color:var(--foreground-muted);font-size:12px;font-weight:400;line-height:1.55}.tool-results-title{color:var(--foreground);font-size:14px;font-weight:700;line-height:1.4}.tool-result-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.tool-result-card{position:relative;overflow:hidden;min-height:96px;border-radius:var(--radius-card);padding:15px;background:var(--surface);border:1px solid var(--border);box-shadow:var(--shadow-card)}.tool-result-card:before{content:"";position:absolute;inset-inline-start:0;top:14px;bottom:14px;width:3px;border-radius:var(--radius-pill);background:var(--surface)}.tool-result-card span{color:var(--foreground-muted);font-size:12px;font-weight:600;line-height:1.4}.tool-result-card b{color:var(--foreground);font-size:clamp(18px,1.7vw,25px);font-weight:700;line-height:1.2;letter-spacing:0;direction:ltr;unicode-bidi:isolate;overflow-wrap:anywhere}.scenario-grid .metric,.portfolio-metric-grid .metric{position:relative;overflow:hidden;border-radius:var(--radius-card);background:var(--surface);border-color:var(--border);box-shadow:var(--shadow-card);padding:14px}.scenario-grid .metric:before,.portfolio-metric-grid .metric:before{content:"";position:absolute;inset-inline-start:0;top:12px;bottom:12px;width:3px;border-radius:var(--radius-pill);background:var(--surface)}.scenario-grid .metric span,.portfolio-metric-grid .metric span{font-size:12px;color:var(--foreground-muted);font-weight:700}.scenario-grid .metric strong,.portfolio-metric-grid .metric strong{font-size:clamp(16px,1.4vw,21px);font-weight:700;direction:ltr;unicode-bidi:isolate;line-height:1.25}
        .trader-dashboard .tool-input{width:100%;max-width:100%;min-width:0;display:grid;grid-template-columns:1fr;gap:9px}.trader-dashboard .tool-input>span,.market-tools-grid .tool-input>span{color:var(--foreground);font-size:13px;font-weight:700;line-height:1.45}.trader-dashboard .tool-input small,.market-tools-grid .tool-input small{color:var(--foreground-muted);font-size:12px;font-weight:400;line-height:1.65}.trader-dashboard .tool-input-shell,.market-tools-grid .market-currency-input{min-width:0;min-height:60px;border-radius:var(--radius-card);border-color:var(--border-strong);background:var(--surface);box-shadow:var(--shadow-card);padding-inline:12px}.trader-dashboard .tool-input-shell:focus-within,.market-tools-grid .market-currency-input:focus-within{border-color:var(--accent);box-shadow:var(--focus-shadow)}.trader-dashboard .tool-input-shell em,.market-tools-grid .market-input-affix{min-width:54px;min-height:38px;display:inline-flex;align-items:center;justify-content:center;border-radius:var(--radius-card);background:var(--surface);border-color:var(--border-strong);color:var(--primary-hover);font-size:12px;font-weight:700;font-variant-numeric:tabular-nums}.trader-dashboard .tool-input .tool-input-shell input,.trader-dashboard .tool-input .tool-input-shell select,.market-tools-grid .market-currency-input input{height:58px!important;font-size:16px!important;font-weight:700!important;font-variant-numeric:tabular-nums;letter-spacing:0;color:var(--foreground)}.trader-dashboard .tool-results{border-radius:var(--radius-panel);padding:16px;background:var(--surface);box-shadow:var(--shadow-card)}.trader-dashboard .tool-results-head{padding-bottom:12px;border-bottom:1px solid var(--border)}.trader-dashboard .tool-result-grid{gap:12px}.trader-dashboard .tool-result-card{isolation:isolate;display:grid;align-content:space-between;gap:12px;min-height:104px;border-radius:var(--radius-panel);padding:16px;background:var(--surface);border-color:var(--border);box-shadow:var(--shadow-card)}.trader-dashboard .tool-result-card span{font-size:12px;font-weight:700;color:var(--foreground-muted);line-height:1.45}.trader-dashboard .tool-result-card b{display:block;max-width:100%;text-align:left;font-size:clamp(18px,2vw,26px);font-weight:700;line-height:1.2;font-variant-numeric:tabular-nums;direction:ltr;unicode-bidi:isolate;overflow-wrap:anywhere}.market-tools-grid .scenario-grid .metric,.portfolio-metric-grid .metric{min-width:0}@media(max-width:520px){.trader-dashboard .tool-input-shell,.market-tools-grid .market-currency-input{min-height:54px;border-radius:var(--radius-card);padding-inline:8px;gap:7px}.trader-dashboard .tool-input-shell em,.market-tools-grid .market-input-affix{min-width:42px;min-height:34px;padding-inline:7px;font-size:12px}.market-scenario-currency-trigger{min-width:48px;height:34px;padding-inline:7px}.trader-dashboard .tool-input .tool-input-shell input,.trader-dashboard .tool-input .tool-input-shell select,.market-tools-grid .market-currency-input input{height:var(--control-h-lg)!important;font-size:15px!important}.trader-dashboard .tool-results{padding:13px;border-radius:var(--radius-panel)}.trader-dashboard .tool-result-card{min-height:88px;padding:14px;border-radius:var(--radius-card)}.trader-dashboard .tool-result-card b{font-size:17px}.trader-dashboard,.trader-tool-card,.tool-result-grid{width:100%;max-width:100%;min-width:0;overflow:hidden}}
        .trader-currency-bar{display:grid;grid-template-columns:minmax(220px,360px) minmax(0,1fr);gap:14px;align-items:end;border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-panel);padding:14px;min-width:0}.trader-currency-action{display:grid;justify-items:start;align-content:end;gap:8px;min-width:0}.trader-currency-action button{min-height:46px;border:1px solid var(--border-strong);border-radius:var(--radius-pill);background:var(--primary);color:var(--primary-hover);padding:0 16px;font:700 12px var(--font-ui);cursor:pointer;box-shadow:var(--shadow-card)}.trader-currency-action button:hover,.trader-currency-action button:focus-visible{outline:none;border-color:var(--accent);box-shadow:var(--focus-shadow)}.trader-currency-action button:disabled{opacity:.72;cursor:not-allowed}.trader-currency-action small{font-size:12px;font-weight:600;line-height:1.45}.trader-currency-action small.success{color:var(--success)}.trader-currency-action small.error{color:var(--danger)}.trader-active-tool{display:grid;gap:16px;min-width:0}.trader-active-intro{display:grid;gap:6px;border:1px solid var(--border);background:var(--accent-soft);border-radius:var(--radius-panel);padding:16px;min-width:0}.trader-active-intro span{color:var(--primary-hover);font-size:12px;font-weight:700;line-height:1.4}.trader-active-intro h3{margin:0;color:var(--foreground);font-size:clamp(20px,2.5vw,28px);font-weight:700;line-height:1.25}.trader-active-intro p{margin:0;color:var(--foreground-muted);font-size:13px;font-weight:400;line-height:1.8}.trader-dashboard .trader-tool-grid{grid-template-columns:minmax(0,1.18fr) minmax(300px,.82fr);gap:16px;align-items:start}.trader-dashboard .trader-tool-card,.trader-dashboard .trader-result-stack{grid-column:auto;min-width:0}.trader-result-stack{display:grid;gap:14px;align-content:start;min-width:0}.tool-formula-card{display:flex;align-items:flex-start;gap:12px;border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-panel);padding:15px;box-shadow:var(--shadow-card);min-width:0}.tool-formula-card>span{width:38px;height:38px;border-radius:var(--radius-card);display:grid;place-items:center;flex:0 0 auto;background:var(--surface);border:1px solid var(--border);color:var(--accent)}.tool-formula-card div{display:grid;gap:6px;min-width:0}.tool-formula-card strong{color:var(--foreground);font-size:14px;font-weight:700;line-height:1.4}.tool-formula-card p{margin:0;color:var(--foreground-muted);font-size:12px;font-weight:400;line-height:1.75}.tool-formula-card b{display:block;width:max-content;max-width:100%;border:1px solid var(--border);background:var(--surface);color:var(--primary-hover);border-radius:var(--radius-pill);padding:7px 10px;font-size:12px;font-weight:700;line-height:1.35;overflow-wrap:anywhere;white-space:normal}.tool-advanced{grid-column:1/-1;border:1px solid var(--border);background:var(--accent-soft);border-radius:var(--radius-card);padding:10px;min-width:0}.tool-advanced summary{display:flex;align-items:center;justify-content:space-between;gap:12px;min-height:42px;color:var(--foreground);font-size:13px;font-weight:700;cursor:pointer;list-style:none}.tool-advanced summary::-webkit-details-marker{display:none}.tool-advanced summary:after{content:"+";width:26px;height:26px;border-radius:var(--radius-pill);display:grid;place-items:center;background:var(--surface);color:var(--primary-hover);font-weight:700}.tool-advanced[open] summary{margin-bottom:12px}.tool-advanced[open] summary:after{content:"âˆ’"}.trader-disclaimer{margin:0;border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-panel);padding:13px 15px;color:var(--foreground-muted);font-size:12px;font-weight:400;line-height:1.8}@media(max-width:1180px){.trader-dashboard .trader-tool-grid,.trader-currency-bar{grid-template-columns:1fr}.trader-dashboard .trader-tool-card,.trader-dashboard .trader-result-stack{grid-column:1/-1}}@media(max-width:720px){.trader-currency-action{justify-items:stretch}.trader-currency-action button{width:100%;min-height:44px}.tool-formula-card{display:grid}.tool-formula-card b{width:100%}.trader-active-intro,.trader-currency-bar{border-radius:var(--radius-card);padding:13px}}
        .trader-premium-dashboard{width:100%;max-width:none;margin-inline:0;display:grid;gap:18px;min-width:0;overflow:hidden}.trader-premium-header{display:flex;align-items:center;gap:16px;min-width:0;border:1px solid var(--border);border-radius:var(--radius-panel);background:var(--surface);box-shadow:var(--shadow-card);padding:clamp(18px,2vw,26px)}.trader-premium-header-icon,.trader-premium-tool-icon,.trader-support-icon,.trader-accordion-icon{width:46px;height:46px;border-radius:var(--radius-card);display:grid;place-items:center;flex:0 0 auto;color:var(--foreground);background:var(--surface);box-shadow:var(--shadow-card)}.trader-premium-header span:not(.trader-premium-header-icon),.trader-premium-main-head span:not(.trader-premium-tool-icon){color:var(--primary-hover);font-size:12px;font-weight:700;line-height:1.35}.trader-premium-header h2,.trader-premium-main-head h3{margin:4px 0 0;color:var(--foreground);font-size:clamp(24px,3.2vw,36px);font-weight:700;line-height:1.2}.trader-premium-header p,.trader-premium-main-head p,.trader-support-card p{margin:7px 0 0;color:var(--foreground-muted);font-size:13px;font-weight:400;line-height:1.8}.trader-premium-layout{display:grid;grid-template-columns:minmax(0,.60fr) minmax(0,1.40fr);grid-template-areas:"support main";gap:18px;align-items:start;min-width:0}.trader-support-column{grid-area:support;display:grid;gap:14px;min-width:0}.trader-premium-main-card{grid-area:main;display:grid;gap:16px;min-width:0;border:1px solid var(--border);border-radius:var(--radius-panel);background:var(--surface);box-shadow:var(--shadow-card);padding:clamp(16px,2vw,24px);overflow:hidden}.trader-support-card{display:flex;gap:13px;min-width:0;border:1px solid var(--border);border-radius:var(--radius-panel);background:var(--surface);box-shadow:var(--shadow-card);padding:16px}.trader-support-card.highlight{background:var(--surface);border-color:var(--border)}.trader-support-icon{width:38px;height:38px;border-radius:var(--radius-card)}.trader-support-card h3{margin:0;color:var(--foreground);font-size:15px;font-weight:700;line-height:1.4}.trader-side-result-grid{display:grid;grid-template-columns:1fr;gap:10px;margin-top:10px}.trader-side-stat{border:1px solid var(--border);border-radius:var(--radius-card);background:var(--surface);padding:11px;display:grid;gap:5px}.trader-side-stat span{color:var(--foreground-muted);font-size:12px;font-weight:700;line-height:1.3}.trader-side-stat b{color:var(--primary-hover);font-size:20px;font-weight:700;line-height:1.2;text-align:start}.trader-steps{margin:10px 0 0;padding-inline-start:20px;color:var(--foreground-muted);display:grid;gap:8px;font-size:13px;font-weight:400;line-height:1.7}.trader-premium-main-head{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:14px;align-items:center;min-width:0;border:1px solid var(--border);border-radius:var(--radius-panel);background:var(--accent-soft);padding:14px}.trader-premium-tool-icon{width:42px;height:42px;border-radius:var(--radius-card)}.trader-premium-main-head h3{font-size:clamp(20px,2.4vw,28px)}.trader-premium-save{display:grid;gap:6px;justify-items:end;min-width:180px}.trader-premium-save button{border:1px solid var(--border-strong);border-radius:var(--radius-pill);background:var(--accent-soft);color:var(--primary-hover);min-height:40px;padding:0 14px;font:700 12px var(--font-ui);cursor:pointer;transition:background .2s ease,transform .2s ease,border-color .2s ease}.trader-premium-save button:hover{background:var(--accent-soft);border-color:var(--border-strong);transform:translateY(-1px)}.trader-premium-save button:disabled{cursor:wait;opacity:.72;transform:none}.trader-premium-save small{font-size:12px;font-weight:600;line-height:1.35}.trader-premium-save small.success{color:var(--success)}.trader-premium-save small.error{color:var(--danger)}.trader-accordion-list{display:grid;gap:12px;min-width:0}.trader-accordion-item{min-width:0;border:1px solid var(--border);border-radius:var(--radius-panel);background:var(--surface);box-shadow:var(--shadow-card);overflow:hidden}.trader-accordion-item.active{background:var(--surface);border-color:var(--border);box-shadow:var(--shadow-card)}.trader-accordion-item>button{width:100%;border:0;background:transparent;color:var(--foreground);display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:12px;text-align:start;padding:14px;cursor:pointer}.trader-accordion-icon{width:38px;height:38px;border-radius:var(--radius-card)}.trader-accordion-copy{display:grid;gap:4px;min-width:0}.trader-accordion-copy strong{color:var(--foreground);font-size:14px;font-weight:700;line-height:1.35}.trader-accordion-copy small{color:var(--foreground-muted);font-size:12px;font-weight:400;line-height:1.55;overflow-wrap:anywhere}.trader-accordion-chevron{width:30px;height:30px;border-radius:var(--radius-pill);display:grid;place-items:center;background:var(--accent-soft);color:var(--primary-hover);font-weight:700;transition:transform .2s ease}.trader-accordion-item.active .trader-accordion-chevron{transform:rotate(180deg)}.trader-accordion-panel{border-top:1px solid var(--border);padding:14px;background:var(--surface);min-width:0}.trader-premium-panel-grid{display:grid;grid-template-columns:minmax(0,1.1fr) minmax(280px,.90fr);gap:14px;align-items:start;min-width:0}.trader-premium-form-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.trader-premium-result-stack{gap:12px}.trader-highlight-result{border:1px solid var(--border);border-radius:var(--radius-panel);background:var(--surface);box-shadow:var(--shadow-card);padding:15px;display:grid;gap:7px}.trader-highlight-result span{color:var(--primary-hover);font-size:12px;font-weight:700}.trader-highlight-result strong{color:var(--success);font-size:clamp(30px,4vw,42px);font-weight:700;line-height:1}.trader-highlight-result p{margin:0;color:var(--foreground-muted);font-size:13px;font-weight:400;line-height:1.75}.trader-highlight-result b{color:var(--foreground)}.trader-premium-disclaimer{display:flex;align-items:flex-start;gap:12px;border:1px solid var(--border);border-radius:var(--radius-panel);background:var(--surface);box-shadow:var(--shadow-card);padding:17px 18px}.trader-premium-disclaimer>span{width:40px;height:40px;border-radius:var(--radius-card);display:grid;place-items:center;color:var(--primary-hover);background:var(--surface);border:1px solid var(--border);flex:0 0 auto}.trader-premium-disclaimer strong{display:block;color:var(--foreground);font-size:14px;font-weight:700;margin-bottom:4px}.trader-premium-disclaimer p{margin:0;color:var(--foreground-muted);font-size:13px;font-weight:400;line-height:1.8}@media(max-width:1180px){.trader-premium-layout{grid-template-columns:1fr;grid-template-areas:"main" "support"}.trader-premium-panel-grid{grid-template-columns:1fr}.trader-premium-main-head{grid-template-columns:auto minmax(0,1fr)}.trader-premium-save{grid-column:1/-1;justify-items:stretch;min-width:0}.trader-premium-save button{width:100%}}@media(max-width:720px){.trader-premium-header,.trader-support-card,.trader-premium-main-card,.trader-premium-disclaimer{border-radius:var(--radius-panel)}.trader-premium-header{align-items:flex-start}.trader-premium-form-grid{grid-template-columns:1fr}.trader-accordion-item>button{grid-template-columns:auto minmax(0,1fr) auto;padding:12px}.trader-accordion-copy small{display:none}.trader-accordion-panel{padding:12px}.trader-premium-main-head{border-radius:var(--radius-card);padding:12px}.trader-premium-header h2{font-size:25px}.trader-side-result-grid{grid-template-columns:1fr}.trader-support-column{order:2}}
        .performance-card-list{display:none}.performance-table-desktop{display:block}.trader-table-wrap{width:100%;max-width:100%;overflow-x:auto;border:1px solid var(--border);border-radius:var(--radius-card);-webkit-overflow-scrolling:touch}.trader-table{display:table;width:100%;min-width:760px;border-collapse:separate;border-spacing:0 8px;padding:8px}.trader-table th{padding:12px 16px;color:var(--foreground-muted);font-size:13px;font-weight:700;line-height:1.35;text-align:start;white-space:nowrap}.trader-table td{padding:13px 16px;background:var(--surface);color:var(--foreground);font-size:13px;font-weight:600;line-height:1.45;white-space:nowrap;vertical-align:middle}.trader-table tbody tr td:first-child{border-start-start-radius:var(--radius-card);border-end-start-radius:var(--radius-card)}.trader-table tbody tr td:last-child{border-start-end-radius:var(--radius-card);border-end-end-radius:var(--radius-card)}.performance-symbol{display:inline-flex;direction:ltr;unicode-bidi:isolate;font-weight:700;letter-spacing:.02em}.performance-value{display:inline-flex;direction:ltr;unicode-bidi:isolate;font-weight:700}.performance-trend{display:inline-flex;align-items:center;justify-content:center;width:max-content;border:1px solid transparent;border-radius:var(--radius-pill);padding:6px 10px;font-size:12px;font-weight:700;line-height:1.2;white-space:nowrap}.performance-trend.bullish{background:var(--success-soft);color:var(--success);border-color:var(--success)}.performance-trend.bearish{background:var(--danger-soft);color:var(--danger);border-color:var(--danger)}.performance-trend.neutral{background:var(--info-soft);color:var(--foreground-muted);border-color:var(--info)}.performance-card{width:100%;max-width:100%;min-width:0;overflow:hidden;border:1px solid var(--border);border-radius:var(--radius-card);background:var(--surface);padding:15px;box-shadow:var(--shadow-card)}.performance-card-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;min-width:0}.performance-card-head div{display:grid;gap:4px;min-width:0}.performance-card-head strong{color:var(--foreground);font-size:17px;font-weight:700;line-height:1.2;direction:ltr;unicode-bidi:isolate;text-align:start}.performance-card-head span{color:var(--foreground-muted);font-size:13px;font-weight:400;line-height:1.45;overflow-wrap:anywhere}.performance-metric-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-top:14px}.performance-metric{min-width:0;border:1px solid var(--border);border-radius:var(--radius-card);background:var(--surface);padding:11px;display:grid;gap:5px}.performance-metric span{color:var(--foreground-muted);font-size:12px;font-weight:600;line-height:1.35}.performance-metric b{color:var(--foreground);font-size:14px;font-weight:700;line-height:1.3;direction:ltr;unicode-bidi:isolate;text-align:start;overflow-wrap:anywhere}.performance-metric b.up,.performance-value.up{color:var(--success)}.performance-metric b.down,.performance-value.down{color:var(--danger)}
        .market-status-card{display:flex;align-items:center;gap:13px;min-width:0;border:1px solid var(--border);border-radius:var(--radius-panel);background:var(--surface);box-shadow:var(--shadow-card);padding:16px}.market-status-card>span{width:42px;height:42px;border-radius:var(--radius-card);display:grid;place-items:center;flex:0 0 auto;background:var(--surface);border:1px solid var(--border);color:var(--accent)}.market-status-card div{display:grid;gap:5px;min-width:0}.market-status-card small{color:var(--foreground-muted);font-size:12px;font-weight:600;line-height:1.35}.market-status-card strong{color:var(--foreground);font-size:18px;font-weight:700;line-height:1.3;min-width:0;overflow-wrap:anywhere}.market-status-banner{display:flex;align-items:flex-start;gap:12px;border:1px solid var(--success);background:var(--success-soft);color:var(--success);border-radius:var(--radius-panel);padding:15px 17px;box-shadow:var(--shadow-card)}.market-status-banner.preparing{border-color:var(--info);background:var(--info-soft);color:var(--primary-hover)}.market-status-banner svg{flex:0 0 auto;margin-top:2px}.market-status-banner strong{display:block;color:inherit;font-size:14px;font-weight:700;line-height:1.4}.market-status-banner p{margin:4px 0 0;color:var(--foreground-muted);font-size:13px;font-weight:400;line-height:1.7}.technical-analysis-panel{width:100%;max-width:100%;border-radius:var(--radius-panel)!important;padding:clamp(16px,2.2vw,28px)!important;background:var(--surface)!important;border-color:var(--border)!important;box-shadow:var(--shadow-card)!important}.technical-analysis-panel .market-section-head{align-items:center;border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-panel);padding:14px;margin-bottom:0}.technical-analysis-panel .market-section-head>svg{width:42px;height:42px;border-radius:var(--radius-card);padding:10px;background:var(--surface);color:var(--foreground);box-shadow:var(--shadow-card)}.technical-selector-shell{background:var(--surface)!important;border-radius:var(--radius-panel)!important;box-shadow:var(--shadow-card)}.technical-category-row,.technical-symbol-row{flex-wrap:wrap;overflow:visible;padding-bottom:2px}.technical-category-row button{min-height:44px;padding:0 17px;font-size:13px}.technical-symbol-pill{min-height:44px}.technical-selected-summary{border:1px solid var(--border-strong);background:var(--surface);border-radius:var(--radius-panel);padding:12px}.technical-summary-item{display:flex;align-items:center;gap:10px;min-width:0;border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-card);padding:13px}.technical-summary-item>span{width:34px;height:34px;border-radius:var(--radius-control);display:grid;place-items:center;flex:0 0 auto;background:var(--surface);color:var(--accent)}.technical-summary-item div{display:grid;gap:4px;min-width:0}.technical-summary-item small{color:var(--foreground-muted);font-size:12px;font-weight:700;line-height:1.35}.technical-summary-item strong{color:var(--foreground);font-size:15px;font-weight:700;line-height:1.35;overflow-wrap:anywhere}.technical-empty-state{display:flex;align-items:flex-start;gap:12px;border:1px solid var(--warning);background:var(--warning-soft);border-radius:var(--radius-panel);padding:16px;color:var(--warning)}.technical-empty-state svg{flex:0 0 auto;margin-top:2px}.technical-empty-state strong{display:block;color:var(--warning);font-size:15px;font-weight:700;line-height:1.5}.technical-empty-state p{margin:5px 0 0;color:var(--warning);font-size:13px;font-weight:400;line-height:1.8}.technical-data-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.technical-data-card{display:grid;gap:12px;align-content:start;min-width:0;border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-panel);padding:16px;box-shadow:var(--shadow-card)}.technical-data-card-head{display:flex;align-items:center;gap:9px;min-width:0}.technical-data-card-head span{width:34px;height:34px;border-radius:var(--radius-control);display:grid;place-items:center;background:var(--surface);color:var(--accent);border:1px solid var(--border);flex:0 0 auto}.technical-data-card h3{margin:0;color:var(--foreground);font-size:14px;font-weight:700;line-height:1.4}.technical-data-card>strong{color:var(--foreground);font-size:23px;font-weight:700;line-height:1.2}.technical-data-rows{display:grid;gap:8px}.technical-data-rows div{display:flex;align-items:center;justify-content:space-between;gap:10px;border:1px solid var(--border);background:var(--surface);border-radius:var(--radius-control);padding:9px 10px}.technical-data-rows small{color:var(--foreground-muted);font-size:12px;font-weight:700}.technical-data-rows b{color:var(--foreground);font-size:13px;font-weight:700}.market-disclaimer{display:flex;align-items:flex-start;gap:12px;padding:17px 18px;color:var(--foreground-muted);background:var(--surface);border-color:var(--border);border-radius:var(--radius-panel)}.market-disclaimer svg{color:var(--primary-hover);flex:0 0 auto}.market-disclaimer strong{display:block;color:var(--foreground);margin-bottom:4px}.market-disclaimer p{margin:0;color:var(--foreground-muted);font-size:13px;line-height:1.7;font-weight:400}
        ...
        @media(max-width:1024px){.technical-category-row,.technical-symbol-row{flex-wrap:nowrap;overflow-x:auto;overflow-y:hidden;padding-bottom:8px}}
        @media(max-width:720px){.market-main{width:100%;max-width:100%;overflow-x:hidden}.market-hero{grid-template-columns:1fr;padding:22px;border-radius:var(--radius-panel)}.market-search-panel,.market-card-grid,.market-status-grid,.market-stat-row,.indicator-list,.scenario-grid,.alert-form,.trader-form-grid,.tool-result-grid{grid-template-columns:1fr}.market-search-panel button{width:100%}.market-search-results{max-height:min(300px,42dvh);top:calc(100% + 10px);border-radius:var(--radius-card)}.market-search-results button{min-height:68px;align-items:flex-start}.market-search-results button small{white-space:normal;text-align:end}.market-hero-card strong{font-size:36px}.market-stock-header{display:grid;gap:14px}.stock-price-block{justify-items:start;text-align:start}.market-panel,.market-card,.market-stock-header{border-radius:var(--radius-card)}.portfolio-card{padding:16px}.compare-bars div{grid-template-columns:42px minmax(0,1fr) 48px}.tool-tabs,.symbol-chip-row{width:100%;max-width:100%;overflow-x:auto;overflow-y:hidden;flex-wrap:nowrap;gap:8px;padding:2px 2px 8px;scrollbar-width:none;-webkit-overflow-scrolling:touch}.tool-tabs::-webkit-scrollbar,.symbol-chip-row::-webkit-scrollbar{display:none}.tool-tabs button,.symbol-chip-row button{flex:0 0 auto;white-space:nowrap}.trader-dashboard{overflow:hidden}.trader-dashboard-head{display:grid;grid-template-columns:auto minmax(0,1fr);align-items:start}.trader-dashboard-head small{grid-column:1/-1;margin-inline-start:0;width:max-content;max-width:100%;white-space:normal}.trader-tool-card{grid-column:1/-1;padding:16px}.trader-tool-card-head{display:grid;grid-template-columns:auto minmax(0,1fr);align-items:start}.tool-reset{grid-column:1/-1;justify-self:start}.economic-calendar-empty{grid-template-columns:1fr;padding:16px;border-radius:var(--radius-panel)}.economic-calendar-empty-icon{width:46px;height:46px;border-radius:var(--radius-card)}.economic-calendar-next,.economic-calendar-event-main{display:grid;justify-content:stretch}.economic-calendar-next-meta{justify-content:flex-start}.economic-calendar-event-metrics{grid-template-columns:1fr}.technical-selector-shell{border-radius:var(--radius-card);padding:12px}.technical-selected-summary{grid-template-columns:repeat(2,minmax(0,1fr))}.technical-result-grid{grid-template-columns:1fr}.technical-category-row,.technical-symbol-row{gap:8px}.technical-category-row button{min-height:38px;padding-inline:13px}.technical-symbol-main{padding-inline:9px;font-size:12px}.performance-card-list{display:grid;grid-template-columns:1fr;gap:12px;width:100%;max-width:100%;min-width:0}.performance-table-desktop{display:none}.performance-card-head{align-items:flex-start}.performance-metric-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.performance-metric{padding:10px}.performance-trend{flex:0 0 auto}}
        ...
        :global(.market-hero){
          ...
          border:1px solid color-mix(in srgb,var(--hero-foreground) 28%,transparent)!important;
        ...
        :global(.market-eyebrow){
          ...
          border:1px solid color-mix(in srgb,var(--hero-foreground) 28%,transparent)!important;
          background:color-mix(in srgb,var(--hero-foreground) 10%,transparent)!important;
          color:var(--hero-foreground)!important;
        ...
        :global(.market-search-panel){
          display:grid!important;
          grid-template-columns:minmax(280px,1.6fr) minmax(160px,.7fr) minmax(150px,.65fr) max-content!important;
        ...
        :global(.market-search-panel .market-search-input-shell:focus-within),
        :global(.market-search-panel select:focus-visible){
        ...
        :global(.market-search-results strong){
          color:var(--foreground)!important}
        :global(.market-search-results button em){
        ...
        :global(.analysis-asset-hero){
          display:grid!important;
          grid-template-columns:minmax(0,1.18fr) minmax(190px,.42fr) minmax(190px,.42fr) auto!important;
          align-items:stretch!important;
          gap:12px!important;
          padding:16px!important;
          border-radius:var(--radius-panel)!important;
        ...
        :global(.analysis-asset-logo){
          ...
          box-shadow:var(--shadow-card)!important}
        :global(.analysis-asset-logo img){
          width:100%!important;
        ...
        :global(.analysis-score-summary){
          display:flex!important;
          align-items:center!important;
          justify-content:space-between!important;
          flex-wrap:wrap!important;
          gap:6px 10px!important}
        :global(.analysis-score-status){
          display:inline-flex!important;
          align-items:center!important;
          gap:5px!important;
          font-weight:700!important}
        :global(.analysis-score-summary.success b),
        :global(.analysis-score-summary.success .analysis-score-status){color:var(--success)!important}
        :global(.analysis-score-summary.danger b),
        :global(.analysis-score-summary.danger .analysis-score-status){color:var(--danger)!important}
        :global(.analysis-reading-card strong){
          color:var(--foreground)!important;
          font-size:22px!important;
          font-weight:700!important;
          line-height:1.15!important}
        :global(.analysis-reading-card.ok){border-color:var(--success)!important;background:var(--success-soft)!important}
        :global(.analysis-reading-card.warn){border-color:var(--warning)!important;background:var(--warning-soft)!important}
        :global(.analysis-reading-card.danger){border-color:var(--danger)!important;background:var(--danger-soft)!important}
        ...
        :global(.analysis-confidence-track i){
          ...
          background:var(--surface-muted)!important}
        :global(.analysis-confidence-track.success i){background:var(--success)!important}
        :global(.analysis-confidence-track.danger i){background:var(--danger)!important}
        ...
        @media(max-width:720px){
          :global(.market-main){gap:18px!important}
          :global(.market-hero){
            border-radius:var(--radius-panel)!important;
            padding:20px!important;
            gap:18px!important}
        ...
        :global(.market-search-submit),
        :global(.market-search-submit svg),
        :global(.ai-regenerate),
        :global(.economic-calendar-filters button[aria-pressed="true"]),
        :global(.tool-tabs button[aria-pressed="true"]),
        :global(.symbol-chip-row button[aria-pressed="true"]),
        :global(.trader-tool-switcher>button[aria-selected='true']),
        :global(.technical-category-row button[aria-pressed="true"]){
          color:var(--primary-foreground)!important}
        :global(.market-search-results button:hover),
        :global(.market-search-results button:focus-visible),
        :global(.market-search-results button[aria-selected="true"]),
        :global(.market-scenario-currency-menu button:hover),
        :global(.market-scenario-currency-menu button:focus-visible),
        :global(.market-scenario-currency-menu button[aria-selected="true"]){
          background:var(--primary-soft)!important;
          border-color:var(--primary)!important;
          color:var(--primary-hover)!important;
          box-shadow:var(--focus-shadow)!important}
        :global(.trader-currency-action button){
          background:var(--primary-soft)!important;
          border-color:var(--primary)!important;
          color:var(--primary-hover)!important}

        :global(.market-price),:global(.stock-price-block strong),:global(.price-chart-values strong),:global(.market-status-value),:global(.analysis-asset-price),:global(.analysis-reading-card strong),:global(.portfolio-metric strong),:global(.tool-result-card strong),:global(.trader-side-stat strong),:global(.performance-value),:global(.performance-metric b),:global(.technical-data-card>strong),:global(.technical-data-rows b){font-family:var(--font-data);font-variant-numeric:tabular-nums}
`}</style>
    </div>
  );
}
