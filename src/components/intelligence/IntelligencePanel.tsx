...
const COPY = {
  ar: {
    ...
    unsupported: 'هذا الأصل غير مدعوم حالياً.', intradayStale: 'لا توجد أسعار حديثة كافية للتحليل داخل اليوم. عند إغلاق السوق تبقى آخر أسعار الجلسة؛ انتظر أسعاراً جديدة أو اختر أفقاً أطول لقراءة وصفية.', partial: 'التحليل جزئي لأن بعض عوامل البيانات غير متاحة.', stale: 'بيانات الأسعار أقدم من الحد المعتمد لهذا الأفق؛ راجع وقت المصدر. لا تصدر توصية حتى تتوفر بيانات مناسبة.',
  ...
  en: {
    ...
    unsupported: 'This asset is not supported yet.', intradayStale: 'No sufficiently recent intraday prices are available. Closed markets retain the last session prices; wait for new observations or choose a longer horizon for descriptive analysis.', partial: 'This analysis is partial because some data factors are unavailable.', stale: 'Price observations exceed this horizon’s freshness limit. Check the source time; a recommendation requires suitable data.',
  ...
  fr: {
    ...
    unsupported: 'Cet actif n’est pas encore pris en charge.', intradayStale: 'Les prix sont trop anciens pour une analyse intrajournalière. Un marché fermé conserve les derniers cours ; attendez de nouvelles observations ou choisissez un horizon plus long pour une lecture descriptive.', partial: 'Cette analyse est partielle car certains facteurs sont indisponibles.', stale: 'Les prix dépassent la limite de fraîcheur de cet horizon. Vérifiez la date de la source ; une recommandation exige des données adaptées.',
...
export function IntelligenceStatusPanel({ result, loading, errorCode, onRetry, emptyMessage, compact = false }: Omit<IntelligencePanelProps, 'showStatus'> & { compact?: boolean }) {
  ...
  if (result.staleData) entries.push({ key: 'stale', tone: 'warning', text: result.horizon === 'INTRADAY' ? copy.intradayStale : copy.stale });
