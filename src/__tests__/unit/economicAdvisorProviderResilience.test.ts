...
const mocks = vi.hoisted(() => ({ user: vi.fn(), rate: vi.fn(), usage: vi.fn(), grounding: vi.fn(), configured: vi.fn(), reply: vi.fn() }));
...
const req = () => new NextRequest('https://example.test/api/economic-intelligence/advisor-chat', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ advisor: 'finance', currency: 'KWD', locale: 'ar', messages: [{ role: 'user', content: 'اشرح التنويع' }] }) });
beforeEach(() => {
  vi.resetAllMocks();
  ...
  mocks.reply.mockResolvedValue({ text: 'اختبار من المزود البديل', provider: 'openai', model: 'gpt-4o-mini' });
});
describe('economic advisor shared provider transport', () => {
  it('returns a real fallback reply while preserving owner-scoped grounding', async () => {
    const response = await POST(req());
    expect(response.status).toBe(200);
    const body = await response.json();
    expect(body).toMatchObject({ ok: true, source: 'ai', provider: 'openai', missing: ['income'] });
  ...
  it('returns truthful 503 when providers fail rather than successful unavailability', async () => {
  ...
  it('does not consume allowance if provider configuration is missing', async () => {
