...
describe('fund holdings evidence without invented fund ratings', () => {
 ...
 it('reports a current verified provider/SSB designation separately from SFM periodic fund monitoring', () => {
   const item=publicCatalogItem(publishedFundRow(),now);
 ...
 it('never lets a published designation hide a persisted explicit fund failure', () => {
   const item=publicCatalogItem(publishedFundRow('non_compliant'),now);
   expect(item.shariahStatus).toBe('non_compliant');
   expect(item.statusLabelAr).toBe('لم يجتز الفحص');
 ...
 it('does not keep a published designation badge after its verification becomes stale', () => {
   const row=publishedFundRow(); row.shariah_last_reviewed_at='2026-09-01T00:00:00Z';
   const item=publicCatalogItem(row,now);
