...
describe('ExistingMarketDataIntelligenceProvider', () => {
  it('recovers normalized history when the quote provider has no candles', async () => {
    ...
    proxyHistoryMock.mockResolvedValue({
      ...
      points: Array.from({ length: 60 }, (_, index) => {
        const close = 0.61 + (index * 0.001);
        return {
          time: new Date(lastPoint - ((59 - index) * 24 * 60 * 60 * 1000)).toISOString(),
          open: close - 0.002,
          high: close + 0.004,
          low: close - 0.004,
          close,
