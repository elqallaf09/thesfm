...
import { normalizeSfmOwnedHistoryRows, readSfmOwnedHistory } from '@/lib/sfm-market/ownedHistory';
...
describe('SFM-owned history API', () => {
  it('does not return internal exception details to callers', async () => {
    const warning = vi.spyOn(console, 'warn').mockImplementation(() => {});
    admin.mockReturnValue({ from: () => { throw new Error('private database connection details'); } });
    try {
      const result = await readSfmOwnedHistory('AAPL');
      expect(result.error).toBe('database_error');
      expect(JSON.stringify(result)).not.toContain('private database');
    } finally { warning.mockRestore(); admin.mockReset(); }
  });
