...
export async function readSfmOwnedHistory(
  ...
  try {
    ...
    console.warn('[sfm-owned-history] read exception', { symbol, code: 'database_error' });
    return { configured: true, source: SFM_OWNED_HISTORY_SOURCE, symbol, points: [], pointCount: 0, externalFallbackUsed: false, generatedAt, error: 'database_error' };
  }
}
