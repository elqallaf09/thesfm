...
describe('valuation snapshot persistence', () => {
  it('refuses incomplete evidence lineage before touching the database', async () => {
    const from = vi.fn();
    await expect(persistValuationSnapshot({ from }, { userId: 'u1', positionId: 'p1', valuation, evidence: evidence.slice(0, 1) })).rejects.toThrow('lineage is incomplete');
    expect(from).not.toHaveBeenCalled();
  });

  it('refuses non-valued results', async () => {
    const from = vi.fn();
    await expect(persistValuationSnapshot({ from }, { userId: 'u1', positionId: 'p1', valuation: { ...valuation, status: 'INSUFFICIENT_EVIDENCE' }, evidence })).rejects.toThrow('Only evidence-backed');
    expect(from).not.toHaveBeenCalled();
  });
});
