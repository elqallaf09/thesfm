...
export async function analyzeRealEstateAsset(
  asset: RealEstateAssetInput,
  outputCurrency: string,
  fxQuotes: FxQuote[] = [],
): Promise<RealEstateAnalystResult> {
  ...
  if (adapters.length === 0) {
    return {
      ...
      sourceFailures: [],
      evidenceCount: 0,
      message: `No verified real-estate evidence adapter is configured for ${asset.countryCode}.`,
    };
  }
  ...
  return {
    ...
    message: valuation.status === 'VALUED'
      ? 'Valuation range is backed by the listed evidence and methodology version.'
      : 'Available evidence is not sufficient for a defensible current valuation.',
  };
}
