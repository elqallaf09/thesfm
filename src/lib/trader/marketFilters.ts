...
export function marketFilterDecision(asset: MarketFilterAsset, selectedMarket: unknown): MarketFilterDecision {
  const marketId = normalizeSelectedMarketKey(selectedMarket);
  if (!marketId) return { allowed: true, marketId: null, reason: 'global_or_unrestricted_market' };
  ...
  const assetType = normalizedAssetType(firstText(asset, ['assetType', 'asset_type', 'quoteType', 'instrumentType', 'finalAssetType']));
