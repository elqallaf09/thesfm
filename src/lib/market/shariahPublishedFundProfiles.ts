...
export function publishedShariahFundCatalogItem(item: ShariahUniverseItem, now = new Date()) {
  const profile = item.assetType === 'etf' ? publishedShariahFundProfile(item.symbol) : null;
  if (!profile) return null;
  const reviewedAt = Date.parse(profile.verifiedAt);
  if (!Number.isFinite(reviewedAt) || reviewedAt > now.getTime() || now.getTime() - reviewedAt >= 7 * 86_400_000) return null;
  const reason = {
