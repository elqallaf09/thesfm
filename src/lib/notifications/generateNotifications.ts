...
export function generateSmartNotifications(data: NotificationSourceData, lang: NotificationLang): SmartNotification[] {
  ...
  (data.marketPriceAlerts ?? []).forEach(row => {
    ...
    if (!symbol) return;
    add({ id: makeId(['market-alert', row.id, status]), title: copy.marketTriggeredTitle, message: copy.marketTriggeredMessage(symbol), type: 'market', severity: 'warning', sourceModule: 'market', sourceId: row.id, actionUrl: '/market-analysis', createdAt: row.created_at });
  });
