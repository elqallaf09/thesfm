...
for (const strip of GLOBAL_MARKET_STRIPS) for (const item of strip.items) {
  if (!item.nameAr) continue;
  arabicNames.set(`${strip.exchangeCode}:${item.symbol}`, item.nameAr);
  // The primary US directory combines domestic venues under one market id.
  if (['NASDAQ','NYSE','AMEX'].includes(strip.exchangeCode || '')) arabicNames.set(`US:${item.symbol}`, item.nameAr);
}
...
export function localizeTvAsset<T extends TvAsset>(asset: T) {
