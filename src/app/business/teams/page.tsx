...
export default function TeamsPage() {
 ...
 async function action(kind: string) {
  ...
  try { const result = await supabase.rpc('sfm_team_action', { p_action: kind, p_value: kind === 'create' ? name.trim() : code.trim() });
 ...
 return <WorkspacePageContainer className={styles.page}><h1>{text.title}</h1><p>{text.note}</p>
  ...
  {userId ? <><form className={styles.card} onSubmit={event => { event.preventDefault(); void action('create'); }}><label className={styles.field}>{text.name}<input required minLength={2} maxLength={80} value={name} onChange={event => setName(event.target.value)} className={styles.input} /></label><Button disabled={busy || name.trim().length < 2}>{text.create}</Button></form>
   <form className={styles.card} onSubmit={event => { event.preventDefault(); void action('join'); }}><label className={styles.field}>{text.code}<input required value={code} maxLength={100} onChange={event => setCode(event.target.value)} className={styles.input} autoComplete="off" /></label><Button disabled={busy || !code.trim()}>{text.join}</Button></form></> : null}
...
function TeamDetail({ team, userId, language, onLeave }: { team: Team; userId: string; language: 'ar' | 'en' | 'fr'; onLeave: () => void }) {
 ...
 const [loader] = useState(() => async () => {
  const [members, notes] = await Promise.all([
   supabase.from('sfm_team_members').select('user_id,role,team_id').eq('team_id',team.id).order('joined_at').limit(100),
 ...
 return <section className={styles.card}><h2>{team.name}</h2>
  ...
  <h3>{text.members}</h3>{data?.members.map(member => <div className={styles.row} key={member.user_id}><span dir="ltr" className={styles.text}>{member.user_id}</span><span>{member.role === 'owner' ? text.owner : text.member}</span>{owner && member.role !== 'owner' ? <Button disabled={busy} variant="outline" onClick={() => void act('remove_member',member.user_id)}>{text.remove}</Button> : null}</div>)}
