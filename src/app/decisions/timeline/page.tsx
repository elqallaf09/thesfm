...
export default function DecisionTimelinePage() {
  ...
  async function resolveEvent(id: string) {
    setResolving(id);
    try {
      const response = await fetch('/api/economic-intelligence/event-outcomes', { method: 'POST', credentials: 'same-origin', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ notificationId: id, action: 'resolved', resolutionCode: 'user_confirmed_resolved' }) });
