...
export async function GET(request: Request) {
  const limited = rateLimitRequest(request, { max: 60, prefix: 'energy-news' });
  if (limited) return limited;

  try {
    const url = new URL(request.url);
    const limit = parseNewsLimit(url.searchParams.get('limit'));
    const payload = await fetchStockCategoryNews('energy', url.searchParams.get('lang'));
    ...
    console.error('[EnergyStocksNews] Failed to load news', {
      message: error instanceof Error ? error.message : String(error),
    });
    return NextResponse.json(
      {
        success: false,
        error: 'Failed to load energy news',
