...
export async function GET(request: NextRequest) {
  ...
  if (!user) return NextResponse.json({ ok: false, error: { code: 'UNAUTHENTICATED' } }, { status: 401, headers: { 'cache-control': 'private, no-store' } });
  ...
  if (!TYPES.has(decisionType)) return NextResponse.json({ ok: false, error: { code: 'INVALID_DECISION_TYPE' } }, { status: 400, headers: { 'cache-control': 'private, no-store' } });
  ...
  if (currency && !/^[A-Z]{3}$/.test(currency)) return NextResponse.json({ ok: false, error: { code: 'INVALID_CURRENCY' } }, { status: 400, headers: { 'cache-control': 'private, no-store' } });
  ...
  try {
    ...
    return NextResponse.json({ ok: false, error: { code: 'DECISION_MEMORY_UNAVAILABLE' } }, { status: 502, headers: { 'cache-control': 'private, no-store' } });
  }
}
