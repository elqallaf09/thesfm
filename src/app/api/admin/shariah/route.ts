...
export const GET = createAdminApiRoute({
  ...
  let query = auth.admin.from('market_symbols').select(CATALOG_COLUMNS)
  ...
  const [rows, countsResult, diagnosticsResult] = await Promise.allSettled([
    query.abortSignal(AbortSignal.timeout(8_000)), computeShariahCounts(auth.admin),
  ...
  return json({ ok: true, items: rows.value.data ?? [], counts, countsError, lastRun: diagnostics?.data ?? null,
...
async function saveOverride({ request, auth, json }: AdminApiContext) {
  ...
  const reviewedBy = auth.access.email || auth.user.email || auth.user.id;
  ...
  const audit = { manualOverride: { updatedAt: new Date().toISOString(), reviewedAt, reviewedBy,
    reviewerUserId: auth.user.id, reviewerEmail: auth.user.email ?? auth.access.email } };
  let lookup = auth.admin.from('market_symbols').select('id,symbol,exchange,shariah_screening_data').eq('symbol', symbol).limit(2);
  ...
  const patch = { shariah_status: status, shariah_reason: reason, shariah_source: source,
    shariah_last_reviewed_at: reviewedAt, shariah_manual_override: true, shariah_reviewed_by: reviewedBy,
    shariah_screening_data: { ...(existing.data?.shariah_screening_data ?? {}), ...audit }, updated_at: new Date().toISOString() };
  const result = existing.data?.id
    ? await auth.admin.from('market_symbols').update(patch).eq('id', existing.data.id).select(CATALOG_COLUMNS).single()
    : await auth.admin.from('market_symbols').insert({ symbol,
      provider_symbol: cleanText(body.providerSymbol ?? body.provider_symbol, 48).toUpperCase() || symbol,
      name: cleanText(body.name, 240) || symbol, asset_type: normalizeAssetType(body.assetType ?? body.asset_type), exchange,
      country: cleanText(body.country, 80) || null, currency: cleanText(body.currency, 12).toUpperCase() || null,
      source: 'manual_admin_seed', is_active: true, ...patch }).select(CATALOG_COLUMNS).single();
  if (result.error) {
...
const writeOptions = { permission: 'admin_dashboard' as const, rateLimit: { max: 30, windowMs: 60_000, prefix: 'admin-shariah-write' } };
