...
export const GET = createAdminApiRoute({
  permission: 'company_reviews',
  rateLimit: { max: 60, windowMs: 60_000, prefix: 'admin-investment-platforms' },
  ...
  if (error) {
    ...
    return json({ ok: false, code: 'PLATFORM_MODERATION_LOAD_FAILED' }, { status: 500 });
  }
...
export const PATCH = createAdminApiRoute({
  permission: 'company_reviews',
  rateLimit: { max: 30, windowMs: 60_000, prefix: 'admin-investment-platform-moderate' },
}, async ({ request, auth, json }) => {
  try {
    const body = await request.json().catch(() => null) as Record<string, unknown> | null;
    const id = typeof body?.id === 'string' ? body.id : '';
    const action = typeof body?.action === 'string' ? body.action : '';
    if (!/^[0-9a-f-]{36}$/i.test(id)) return json({ ok: false, code: 'INVALID_PLATFORM_ID' }, { status: 400 });
    ...
    const { data: before, error: loadError } = await auth.admin
      ...
      .eq('id', id)
      .single();
    if (loadError || !before) return json({ ok: false, code: 'PLATFORM_NOT_FOUND' }, { status: 404 });

    if (action === 'merge') {
      const targetId = typeof body?.targetId === 'string' ? body.targetId : '';
      if (!/^[0-9a-f-]{36}$/i.test(targetId) || targetId === id) return json({ ok: false, code: 'INVALID_MERGE_TARGET' }, { status: 400 });
      ...
      if (error) {
        ...
        return json({ ok: false, code: 'PLATFORM_MERGE_FAILED' }, { status: 409 });
      }
      return json({ ok: true, action: 'merge' });
    }

    if (!['approve', 'reject', 'disable', 'update'].includes(action)) return json({ ok: false, code: 'INVALID_MODERATION_ACTION' }, { status: 400 });
    ...
    const { data: duplicate } = await auth.admin
      ...
      .neq('id', id)
      .maybeSingle();
    if (duplicate) return json({ ok: false, code: 'PLATFORM_DUPLICATE', duplicate: { id: duplicate.id, name: duplicate.canonical_name, status: duplicate.status } }, { status: 409 });
    ...
    if (error || !updated) {
      ...
      return json({ ok: false, code: 'PLATFORM_MODERATION_FAILED' }, { status: 500 });
    }
    ...
    if (error instanceof PlatformValidationError) return json({ ok: false, code: error.code }, { status: 400 });
