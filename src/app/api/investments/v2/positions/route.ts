import { NextRequest, NextResponse } from 'next/server';
import {
  InvestmentCanonicalNotReadyError,
  loadCanonicalInvestmentPositions,
} from '@/lib/investments/canonicalPositions.server';
import { getCurrentUserFromRequest } from '@/lib/server/adminAccess';
import { checkRateLimitWithMetadata } from '@/lib/server/rateLimiter';

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

const NO_STORE = { 'cache-control': 'private, no-store' };

export async function GET(request: NextRequest) {
  const user = await getCurrentUserFromRequest(request).catch(() => null);
  if (!user) {
    return NextResponse.json(
      { ok: false, error: { code: 'UNAUTHENTICATED' } },
      { status: 401, headers: NO_STORE },
    );
  }

  const limit = checkRateLimitWithMetadata(`user:${user.id}`, {
    max: 30,
    windowMs: 60_000,
    prefix: 'investment-v2-positions',
  });
  if (!limit.allowed) {
    return NextResponse.json(
      { ok: false, error: { code: 'APPLICATION_RATE_LIMITED' } },
      {
        status: 429,
        headers: { ...NO_STORE, 'Retry-After': String(Math.max(1, limit.retryAfterSeconds)) },
      },
    );
  }

  try {
    const result = await loadCanonicalInvestmentPositions(user.id);
    return NextResponse.json(
      {
        ok: true,
        positions: result.positions,
        readiness: result.readiness,
      },
      { headers: NO_STORE },
    );
  } catch (error) {
    if (error instanceof InvestmentCanonicalNotReadyError) {
      return NextResponse.json(
        {
          ok: false,
          error: { code: error.code },
          readiness: error.readiness,
        },
        { status: 409, headers: NO_STORE },
      );
    }

    return NextResponse.json(
      { ok: false, error: { code: 'INVESTMENT_POSITIONS_UNAVAILABLE' } },
      { status: 502, headers: NO_STORE },
    );
  }
}
