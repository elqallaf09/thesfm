...
PR #185 shipped as `67f62dd18176eb1925eaa76c34104662b2d20de9`, Production `dpl_D8t84f32EFVQApCTLqA7nnhTRH91` READY. Exact candidate `625d193` passed [core CI](https://github.com/elqallaf09/thesfm/actions/runs/35340380055): 2,976 unit tests, 93 engine tests, 861 smoke tests (four passed on retry; 100 skipped), and 39 performance tests (two skipped). Real isolation passed 230 checks; the 153-migration chain and missing-storage recovery drill passed. Production observation ran 12:20:11–12:25:40 UTC September 18. Saudi/Qatar first twelve results had quotes with retained source times; ADX/Egypt unavailable states stayed explicit; protected anonymous routes denied access. The later optional authenticated Preview job failed before browser assertions because merge deleted its Supabase branch. This is not an all-green workflow claim. Detailed evidence and remaining chat/source observations are in [PR #185](https://github.com/elqallaf09/thesfm/pull/185).
...
### Confirmed production storage gap and forward recovery

A direct production catalog check found `investment_valuation_evidence`, `investment_valuation_snapshots`, `investment_snapshot_evidence`, `persist_investment_valuation_snapshot(uuid,uuid,jsonb,jsonb,jsonb)`, and `sfm_market_observations` absent. Repository migration tests alone had not established that these objects existed in Production. This blocks the intended property evidence/history and durable market-observation storage even though their application code is present.

Applied append-only recovery `20260918112815_recover_missing_evidence_storage`. It embeds the published canonical definitions, restores a wholly missing property schema and observation store, rejects a partially missing property schema for review, and preserves existing objects/data. It does not enable an unqualified valuation adapter or seed invented observations. Published historical migration files remain unchanged.

Before production application, the same SQL passed an actual missing-schema recovery drill and an idempotency check on the task-owned schema-only Supabase PR branch. That branch had zero users, stored objects and evidence/observation rows. Object IDs stayed unchanged on the second application. Production then confirmed all four tables with RLS; property reads are owner-scoped, browser writes are denied, market observations are service-only, and the atomic writer remains an invoker function callable only by service_role.

CI now drops only these empty stores in the named disposable database, runs the recovery, and executes the existing two-user property RLS, lineage and atomic rollback assertions under permissive and closed defaults. The complete clean chain exercises the already-present path before that drill.

The production migration ledger predates several repository migrations; this recovery repairs confirmed runtime schema absence without falsely marking unrelated old migrations applied. The existing main-branch Supabase migration-history drift remains an operational follow-up, separate from the explicit storage/schema and permission checks above. The task-owned Preview was reset after its temporary drill migrations so its ledger follows the final repository chain.

The missing production storage was found after the second application Preview had built. It is included in this same release and requires the final genuine Vercel status for the amended head; development-branch deployment remains disabled.
