...
| Browser traces showed a late watchlist response replacing the focused stock button just after the drawer closed. | Preserve the stock trigger's focus across background data renders. | Existing cold-symbol retry/close flows, retaining the focus assertion. |
