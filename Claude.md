...
- 2026-07-29: Fixed the duplicated AI Analyst workspace tree (Playwright strict-mode: two `<main data-testid="ai-analyst-workspace">`, dir="ltr" + dir="rtl") seen on the authenticated Vercel Preview suite. Root cause: routes stream their page segment under the root `loading.tsx` Suspense boundary; any client state commit that reaches consumers inside a still-pending segment (locale flip from `sfm_lang`, auth restore, currency/density/mobile first-paint syncs) makes React abandon the segment's hydration and client-render it (`retrySuspenseComponentWithoutHydrating` — transitions included), and the late-arriving server HTML is left orphaned in the DOM. Fix: new `src/lib/runtime/streamingHydration.ts` (`commitWhenStreamSettled`) defers first-paint provider commits until the document finished loading and all pending segment reveal markers (`template[id^="B:"]`) are gone, then commits at transition priority; applied in `LanguageProvider`, `PublicLanguageProvider`, `useAuth` (initial restore + auth-state changes), `useCurrency`, `useDensity`, and `use-mobile`. Reproduced pre-fix at 6–9/18 page loads with CDP-throttled streaming (25–100 KB/s); 0/18 post-fix in both the locale-flip (en) and no-flip (ar) scenarios. Guarded by the streaming-safety contract in `aiAnalystWorkspaceConsolidation.test.ts` and the single-tree/locale/refresh regression test in `tests/smoke/ai-analyst-consolidation.spec.ts`.
