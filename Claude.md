...
- 2026-07-12: Workspace architecture phase 3 — added a typed workspace registry (`src/config/workspaces/`: types, registry, resolver, navigation mapping) organizing the platform into four workspaces (personal-finance `/dashboard`, markets-trading `/market-analysis`, companies-services `/investment-companies`, administration `/sfm-admin-control`). Pathnames resolve via longest-prefix, segment-aware matching with a personal-finance fallback; `isPublicShellRoute` is now the single source AppLayout consumes. Sidebar and MobileMenu filter the unchanged `NAV_GROUPS` per active workspace and render the accessible `WorkspaceSwitcher` (admin entry hidden from non-admins — presentation only; server gates unchanged). The trader terminal keeps its specialized iframe shell and gained `target="_top"` exit links back to `/market-analysis`. No routes moved, no URLs changed, no middleware/auth changes, nothing persisted. Details: docs/workspace-architecture.md; tests: workspaceRegistry.test.ts.
